...
import {
    ...
    SetHideProgress,
    SetShowProgress,
} from '../../../shared/modules/universal-clipboard/state/clipboard.state';
...
export class DashboardComponent implements OnInit, OnDestroy {
    ...
    @Select(WidgetsRawdataState.getLastModifiedWidgetRawdataByGroup)
    widgetGroupRawData$: Observable<any>;
    @Select(AppShellState.getCurrentMediaQuery) mediaQuery$: Observable<string>;
    @Select(DBSettingsState.GetDashboardMode)
    dashboardMode$: Observable<string>;
    @Select(NavigatorState.getDrawerOpen) drawerOpen$: Observable<any>;
    @Select(EventsState.GetEvents) events$: Observable<any>;

    @ViewChild('tplVariablePanel')
    tplVariablePanel: TemplateVariablePanelComponent;
    // available widgets menu trigger
    @ViewChild('availableWidgetsMenuTrigger', { read: MatMenuTrigger })
    availableWidgetsMenuTrigger: MatMenuTrigger;
    ...
    // portal templates
    @ViewChild('dashboardNavbarTmpl', { static: true })
    dashboardNavbarTmpl: TemplateRef<any>;

    @ViewChild(DboardContentComponent, {
        read: DboardContentComponent,
        static: true,
    })
    dbContent: DboardContentComponent;

    @ViewChild(NavbarClipboardMenuComponent, {
        read: NavbarClipboardMenuComponent,
    })
    clipboardMenu: NavbarClipboardMenuComponent;
    ...
    ngOnInit() {
        ...
        this.subscription.add(
            this.activatedRoute.url.subscribe((url) => {
                ...
                this.tplVariables = {
                    editTplVariables: { tvars: [] },
                    viewTplVariables: { tvars: [] },
                    scopeCache: [],
                };
                if (this.tplVariablePanel) {
                ...
                this.snapshot =
                    this.activatedRoute.snapshot.pathFromRoot[1].routeConfig
                        .path === 'snap'
                        ? true
                        : false;
                ...
                if (
                    this.clipboardMenu &&
                    this.clipboardMenu.getDrawerState() === 'opened'
                ) {
        ...
        // setup navbar portal
        this.dashboardNavbarPortal = new TemplatePortal(
            this.dashboardNavbarTmpl,
            undefined,
            {},
        );
            ...
            this.interCom.requestListen().subscribe((message: IMessage) => {
                ...
                switch (message.action) {
                    ...
                    case 'getWidgetCachedData':
                        ...
                        if (
                            widgetCachedData &&
                            widgetCachedData['error'] !== undefined
                        ) {
                            hasQueryError = true;
                        }
                        ...
                        if (
                            !widgetCachedData ||
                            Object.keys(widgetCachedData).length === 0 ||
                            hasQueryError
                        ) {
                            this.handleQueryPayload(message);
                        } else {
                            this.updateWidgetGroup(
                                message.id,
                                widgetCachedData,
                            );
                        }
                    ...
                    case 'setDashboardEditMode':
                        ...
                        const wdIdx = this.widgets.findIndex(
                            (w) => w.id === message.id,
                        );
                        if (this.widgets[wdIdx]) {
                            this.editViewModeMeta.title =
                                this.widgets[wdIdx].settings.title;
                        }
                        // copy the widget data to editing widget
                        if (message.id) {
                            this.wData[this.editViewModeMeta.id] =
                                this.wData[message.id];
                        }
                        /* eslint-disable max-len */
                        this.dbWdViewBackup = this.utilService.deepClone({
                            time: this.dbTime,
                            tot: this.dbToT,
                            downsample: this.dbDownsample,
                            isDBZoomed: this.isDBZoomed,
                        });
                        // when click on view/edit mode, update db setting state of the mode
                        // need to setTimeout for next tick to change the mode
                        setTimeout(() => {
                            this.store.dispatch(
                                new UpdateMode(message.payload),
                            );
                        });
                        break;
                    case 'removeWidget':
                        const deleteWidgetIdx = this.widgets.findIndex(
                            (w) => w.id === message.payload.widgetId,
                        );
                        if (deleteWidgetIdx > -1) {
                            const deleteWidget = this.utilService.deepClone(
                                this.widgets[deleteWidgetIdx],
                            );
                            this.updateTplVariableForCloneDelete(
                                deleteWidget,
                                'delete',
                            );
                        }
                        this.store.dispatch(
                            new DeleteWidget(message.payload.widgetId),
                        );
                    ...
                    case 'cloneWidget':
                        ...
                        const cloneWidget = JSON.parse(
                            JSON.stringify(message.payload),
                        );
                        cloneWidget.id = this.utilService.generateId(
                            6,
                            this.utilService.getIDs(this.widgets),
                        );
                        cloneWidget.gridPos.yMd =
                            cloneWidget.gridPos.yMd + cloneWidget.gridPos.h;
                        for (let i = 0; i < this.widgets.length; i++) {
                            if (
                                this.widgets[i].gridPos.yMd >=
                                cloneWidget.gridPos.yMd
                            ) {
                                this.widgets[i].gridPos.yMd +=
                                    cloneWidget.gridPos.h;
                            }
                        }
                        ...
                        this.store.dispatch(
                            new UpdateWidgets(
                                this.utilService.deepClone(this.widgets),
                            ),
                        );
                        this.rerender = { reload: true };
                        gridsterContainerEl =
                        ...
                        cloneWidgetEndPos =
                            ...
                            this.gridsterUnitSize.height;
                        containerPos =
                            gridsterContainerEl.getBoundingClientRect();
                        if (cloneWidgetEndPos > containerPos.height) {
                            setTimeout(() => {
                                gridsterContainerEl.scrollTop =
                                    cloneWidgetEndPos - containerPos.height;
                            }, 100);
                        }
                        this.updateTplVariableForCloneDelete(
                            cloneWidget,
                    ...
                    case 'pasteClipboardWidgets':
                        // widgets = this.widgets;
                        const clipboardWidgets = JSON.parse(
                            JSON.stringify(message.payload),
                        );
                        let batchGridPosOffset: any = 0;

                        // get existing Ids
                        const excludedIds: any[] = this.utilService.getIDs(
                            this.widgets,
                        );
                        ...
                        for (let i = 0; i < clipboardWidgets.length; i++) {
                            ...
                            const id: any = this.utilService.generateId(
                                6,
                                excludedIds,
                            );
                            ...
                            if (this.widgets.length > 0) {
                                for (let j = 0; j < this.widgets.length; j++) {
                                    if (
                                        this.widgets[j].gridPos.yMd >=
                                        clipboardWidgets[i].gridPos.yMd
                                    ) {
                                        this.widgets[j].gridPos.yMd +=
                                            clipboardWidgets[i].gridPos.h;
                                    }
                                }
                            }
                        ...
                        cloneWidgetEndPos =
                            batchGridPosOffset * this.gridsterUnitSize.height;
                        containerPos =
                            gridsterContainerEl.getBoundingClientRect();
                        if (cloneWidgetEndPos > containerPos.height) {
                            setTimeout(() => {
                    ...
                    case 'changeWidgetType':
                        this.iiService.closeIsland();
                        const [newConfig, needRefresh] =
                            this.wdService.convertToNewType(
                                message.payload.newType,
                                message.payload.wConfig,
                            );
                        const wId = this.snapshot
                            ? message.id
                            : this.editViewModeMeta.id;
                    ...
                    case 'downloadWidgetData':
                        ...
                        break;
                    case 'getQueryData':
                        this.notifyWidgetLoaderUserHasWriteAccess(
                            this.writeSpaces.length > 1,
                        );
                        this.handleQueryPayload(message);
                        this.rerender = { reload: true };
                        break;
                    ...
                    case 'updateWidgetConfig':
                        let applyTpl = false;
                        const mIndex = this.widgets.findIndex(
                            (w) => w.id === message.id,
                        );
                        if (mIndex === -1) {
                            ...
                            this.widgets = this.dbService.positionWidgetY(
                                this.widgets,
                                newWidgetY,
                            );
                            // change name to first metric if name is not changed
                            if (
                                message.payload.widget.settings
                                    .component_type !==
                                    'MarkdownWidgetComponent' &&
                                message.payload.widget.settings
                                    .component_type !== 'EventsWidgetComponent'
                            ) {
                                if (
                                    message.payload.widget.settings.title ===
                                        'my widget' &&
                                    message.payload.widget.queries[0].metrics
                                        .length
                                ) {
                                    message.payload.widget.settings.title =
                                        message.payload.widget.queries[0].metrics[0].name;
                                }
                            }
                            ...
                            if (
                                ...
                                this.widgets[0].settings.component_type ===
                                ...
                                this.widgets[mIndex].settings.component_type !==
                                message.payload.widget.settings.component_type
                            ) {
                        ...
                        if (applyTpl) {
                            const sub = this.store
                                ...
                                .subscribe((res) => {
                                    const tplVars = this.variablePanelMode.view
                                        ? this.tplVariables.viewTplVariables
                                            .tvars
                                        : this.tplVariables.editTplVariables
                                            .tvars;
                                    this.applyTplToNewWidget(
                            ...
                            this.dbService
                                ...
                                .subscribe((results) => {
                                    this.interCom.responsePut({
                                        ...
                                        payload: {
                                            tplVariables: this.variablePanelMode
                                                .view
                                                ? this.tplVariables
                                                    .viewTplVariables
                                                : this.tplVariables
                    ...
                    case 'SaveSnapshot':
                        ...
                        const dbState = this.utilService.deepClone(
                            this.store.selectSnapshot(DBState),
                        );
                        ...
                        if (dbState.Settings.tplVariables) {
                            dbState.Settings.tplVariables.tvars =
                                this.tplVariables.viewTplVariables.tvars.filter(
                                    (d) =>
                                        aliases.includes('[' + d.alias + ']') ||
                                        aliases.includes('![' + d.alias + ']'),
                                );
                        }
                        delete message.payload.widget.settings.time
                            .overrideTime;
                        dbState.Widgets.widgets = [message.payload.widget];
                        const dbcontent =
                            this.dbService.getStorableFormatFromDBState(
                                dbState,
                            );
                        dbcontent.settings.time.start = this.editViewModeMeta
                            .queryDataRange
                            ? this.editViewModeMeta.queryDataRange.start
                            : this.wdMetaData[message.id].queryDataRange.start;
                        dbcontent.settings.time.end = this.editViewModeMeta
                            .queryDataRange
                            ? this.editViewModeMeta.queryDataRange.end
                            : this.wdMetaData[message.id].queryDataRange.end;
                        ...
                        if (this.dbid !== '_new_') {
                            payload.sourceType = this.snapshot
                                ? 'SNAPSHOT'
                                : 'DASHBOARD';
                    ...
                    case 'dashboardSaveRequest':
                        ...
                        if (this.snapshot) {
                            this.store.dispatch(
                                new UpdateWidgets([this.newWidget]),
                            );
                        }

                        if (message.payload.updateFirst === true) {
                            this.store.dispatch(
                                new UpdateDashboardTitle(message.payload.name),
                            );
                        }
                        ...
                        this.store
                            .dispatch(new SaveDashboard(this.dbid, payload2))
                            .subscribe(() => {
                                if (this.oldFullPath.length > 0) {
                                    const oldDbFile: any =
                                        this.store.selectSnapshot(
                                            DbfsResourcesState.getFile(
                                                this.oldFullPath,
                                            ),
                                        );
                                    ...
                                    if (
                                        oldDbFile.name !==
                                        dbcontent2.settings.meta.title
                                    ) {
                                        const folder = this.store.selectSnapshot(
                                            DbfsResourcesState.getFolder(
                                                oldDbFile.parentPath,
                                            ),
                                        );
                                        this.store.dispatch(
                                            new DbfsRefreshFolder(
                                                folder.fullPath,
                                                {},
                                            ),
                                        );
                    ...
                    case 'updateTemplateVariables':
                        this.store.dispatch(
                            new UpdateVariables(message.payload),
                        );
                        break;
                    ...
                    case 'updateDashboardSettings':
                        if (message.payload.meta) {
                            this.store.dispatch(
                                new UpdateMeta(message.payload.meta),
                            );
                        }
                        break;
                    case 'GetTplVariables':
                        ...
                        this.interCom.responsePut({
                            action: 'TplVariables',
                            payload: {
                                tplVariables: this.variablePanelMode.view
                                    ? this.tplVariables.viewTplVariables
                                    : this.tplVariables.editTplVariables,
                            },
                        });
                    ...
                    case 'SetZoomDateRange':
                        ...
                        if (message.payload.isZoomed) {
                            /* eslint-disable max-len */
                            const start = this.dateUtil
                                .timeToMoment(
                                    this.dbTime.start,
                                    this.dbTime.zone,
                                )
                                .unix();
                            const end = this.dateUtil
                                .timeToMoment(this.dbTime.end, this.dbTime.zone)
                                .unix();
                            message.payload.start =
                                message.payload.start !== -1
                                    ...
                                    : start;
                            message.payload.end =
                                message.payload.end !== -1
                                    ? message.payload.end
                                    : end;

                            const newStart = this.dateUtil.timestampToTime(
                                message.payload.start,
                                this.dbTime.zone,
                            );
                            const newEnd = this.dateUtil.timestampToTime(
                                message.payload.end,
                                this.dbTime.zone,
                            );
                            ...
                            const dbSettings =
                                this.store.selectSnapshot(DBSettingsState);
                            this.dbTime = { ...dbSettings.time };
                            this.dbTime.start = isNaN(dbSettings.time.start)
                                ? this.dbTime.start
                                : this.dateUtil.timestampToTime(
                                    dbSettings.time.start,
                                    this.dbTime.zone,
                                );
                            this.dbTime.end = isNaN(dbSettings.time.start)
                                ? this.dbTime.end
                                : this.dateUtil.timestampToTime(
                                    dbSettings.time.end,
                                    this.dbTime.zone,
                        ...
                        this.interCom.responsePut({
                            ...
                            id: this.viewEditMode
                                ? this.editViewModeMeta.id
                                : '',
                            payload: {
                                zoomingWid: message.id,
                                overrideOnly: overrideOnly,
                                date: {
                                    ...message.payload,
                                    zone: this.dbTime.zone,
                                },
                            },
                        });
                    ...
                    case 'copyWidgetToClipboard':
                        ...
                        setTimeout(() => {
                            const dbData = this.store.selectSnapshot(
                                DBState.getLoadedDB,
                            );
                            const widgetCopy: any = {
                                ...message.payload.widget,
                            };
                            ...
                            widgetCopy.settings.clipboardMeta = {
                                ...
                                copyDate: Date.now(),
                                referencePath:
                                    dbData.path + '@' + widgetCopy.id,
                                preview: message.payload.preview,
                            };

                            const resolvedWidgets: any[] =
                                this.resolveDbTplVariablesForClipboard([
                                    widgetCopy,
                                ]);

                            if (
                                this.clipboardMenu.getDrawerState() === 'closed'
                            ) {
                                this.clipboardMenu.toggleDrawerState({});
                            }

                            this.store.dispatch(
                                new ClipboardAddItems(resolvedWidgets),
                            );
                        }, 50);
                        break;
                    case 'batchItemUpdated':
                        this.batchSelectedItems[message.id] =
                            message.payload.selected;
                        this.checkBatchSelectAll();
                        break;
                    case 'newFromClipboard':
                        this.newFromClipboard = true;
                        const cbWidgetItems = JSON.parse(
                            JSON.stringify(message.payload),
                        );
                        ...
                        for (let i = 0; i < cbWidgetItems.length; i++) {
                            const item: any = cbWidgetItems[i];
                            const id = this.utilService.generateId(6, excludeIds);
                            item.id = id;
                            excludeIds.push(id);
                        }
        ...
        this.subscription.add(
            this.mediaQuery$.subscribe((currentMediaQuery) => {
        ...
        this.subscription.add(
            this.loadedRawDB$.subscribe((db) => {
                ...
                if (dbstate.loaded) {
                    ...
                    this.store
                        .dispatch(
                            new LoadDashboardSettings({
                                ...db.content.settings,
                                mode: this.snapshot ? 'snap' : 'dashboard',
                            }),
                        )
                        .subscribe(() => {
                            // update WidgetsState after settings state sucessfully loaded
                            this.store.dispatch(
                                new UpdateWidgets(db.content.widgets),
                            );
                        });
                    ...
                    this.checkUserFavorited$ = this.store.select(
                        DbfsResourcesState.checkFileFavorited(this.dbid),
                    );
                    this.checkUserFavoritedSub =
                        this.checkUserFavorited$.subscribe((val) => {
            ...
            this.dbPath$.subscribe((path) => {
                ...
                if (path !== '_new_' && path !== undefined) {
                    ...
                    if (this.dbid === '_new_') {
                        ...
                        if (path !== '/_new_' && path !== undefined) {
                            fullPath =
                                '/' + path.split('/').slice(2).join('/'); // strip off the id part of the url
                            this.oldFullPath = this.fullPath;
                            this.fullPath = fullPath;
                            const details =
                                this.dbfsUtils.detailsByFullPath(fullPath);
                            const parentDetails =
                                this.dbfsUtils.detailsByFullPath(
                                    details.parentPath,
                                );
                            if (parentDetails.topFolder) {
                                this.store.dispatch(
                                    new DbfsLoadTopFolder(
                                        parentDetails.type,
                                        parentDetails.typeKey,
                                        {},
                                    ),
                                );
                            } else {
                                this.store.dispatch(
                                    new DbfsLoadSubfolder(
                                        parentDetails.fullPath,
                                        {},
                                    ),
                                );
                            }
        ...
        this.subscription.add(
            this.dbStatus$.subscribe((status) => {
                switch (status) {
                    case 'save-success':
                    ...
                    case 'delete-success':
                        this.snackBar.open(
                            'Dashboard has been moved to trash folder.',
                            '',
                            {
                                horizontalPosition: 'center',
                                verticalPosition: 'top',
        ...
        this.subscription.add(
            this.snapshotId$.subscribe((id) => {
                if (
                    id &&
                    (!this.snapshot || (this.snapshot && id !== this.dbid))
                ) {
                    ...
                    this.subscription.add(
                        this.dbService
                            .resolveDBScope(
                                this.tplVariables,
                                widgets,
                                this.variablePanelMode,
                            )
                            .subscribe((scopes) => {
                                this.tplVariables.scopeCache = scopes;
                                // sort widget by grid row, then assign
                                this.widgets =
                                    this.utilService.deepClone(widgets);
                                if (!this.snapshot) {
                                    this.widgets.sort(
                                        (a, b) =>
                                            a.gridPos.y - b.gridPos.y ||
                                            a.gridPos.x - b.gridPos.x,
                                    );
                                    // batch
                                    for (
                                        let i = 0;
                                        ...
                                        const item: any = this.widgets[i];
                                        if (!this.batchSelectedItems[item.id]) {
                                            this.batchSelectedItems[item.id] =
                                                false;
                                        }
                                    }
                                } else {
        ...
        this.subscription.add(
            this.dashboardMode$.subscribe((mode) => {
                this.viewEditMode =
                    !mode || mode === 'dashboard' ? false : true;
                this.cdkService.setNavbarClass(
        ...
        this.subscription.add(
            this.dbTime$.subscribe((t) => {
                const timeZoneChanged =
                    (this.isDBZoomed && this.dbTime.zone !== t.zone) ||
                    (this.dbTime.start === t.start &&
                        this.dbTime.end === t.end &&
                ...
                if (timeZoneChanged) {
                    ...
                    this.isDBZoomed = false;
                    this.dbTime = { ...t };
                    this.dbTime.start = isNaN(t.start)
                        ? this.dbTime.start
                        : this.dateUtil.timestampToTime(
                            t.start,
                            this.dbTime.zone,
                        );
                    this.dbTime.end = isNaN(t.start)
                        ? this.dbTime.end
                        : this.dateUtil.timestampToTime(
                            t.end,
                            this.dbTime.zone,
        ...
        this.subscription.add(
            this.meta$.subscribe((t) => {
                ...
                if (
                    ...
                    this.meta.title &&
                    Object.keys(this.oldMeta).length === 0
                ) {
                    this.oldMeta = { ...this.meta };
                }
                if (this.meta.title) {
        ...
        this.subscription.add(
            this.downSample$.subscribe((downsample) => {
                this.dbDownsample = {
                    ...this.utilService.deepClone(downsample),
                };
            }),
        );
        this.subscription.add(
            this.tot$.subscribe((tot) => {
                this.dbToT = tot ? this.utilService.deepClone(tot) : {};
                if (
                    this.isToTChanged &&
                    !deepEqual(this.dbPrevToT, this.dbToT) &&
                    ((this.dbToT.value && this.dbToT.period) ||
                        (this.dbPrevToT.value &&
                            this.dbPrevToT.period &&
                            (this.dbToT.value || this.dbToT.period)))
                ) {
                    this.isToTChanged = false;
                    this.applyToTChange();
        ...
        this.subscription.add(
            this.tplVariables$.subscribe((tpl) => {
                // whenever tplVariables$ trigger, we save to view too.
                if (tpl) {
                    this.tplVariables.editTplVariables =
                        this.utilService.deepClone(tpl);
                    this.tplVariables.viewTplVariables =
                        this.utilService.deepClone(tpl);
                    // if there is override then do this, only at first apply
                    if (tpl.override) {
                        this.tplVariables.viewTplVariables.tvars =
                            this.utilService.deepClone(tpl.override);
                        ...
                        const tagOverrides =
                            this.urlOverrideService.getTagOverrides() || {};
                        // eslint-disable-next-line guard-for-in
                        for (const alias in tagOverrides) {
                            const idx =
                                this.tplVariables.viewTplVariables.tvars.findIndex(
                                    (t) => t.alias === alias,
                                );
                            if (idx > -1) {
                                this.tplVariables.viewTplVariables.tvars[
                                    idx
                                ].filter = tagOverrides[alias];
                            }
                        }
                    }
        ...
        this.subscription.add(
            this.userFolderData$.subscribe((result: any) => {
                if (result && result.loaded) {
                    const namespaceCopy = this.utilService.deepClone(
                        result.namespaces,
                    );
                    namespaceCopy.sort((a: any, b: any) => this.utilService.sortAlphaNum(a.name, b.name));
                    this.userNamespaces = namespaceCopy;

                    if (
                        result.personalFolders &&
                        result.personalFolders[0] &&
                        result.personalFolders[0].fullPath
                    ) {
                        this.user = this.getOwnerFromPath(
                            result.personalFolders[0].fullPath,
                        );
                    }
        ...
        this.subscription.add(
            this.drawerOpen$.pipe(skip(1)).subscribe((sideNav) => {
                setTimeout(() => {
                    this.rerender = { reload: true };
                }, 300);
            }),
    ...
    private handleWidgetsLoad(widgets: any) {
        ...
        if (!this.snapshot) {
            this.widgets.sort(
                (a, b) =>
                    a.gridPos.y - b.gridPos.y || a.gridPos.x - b.gridPos.x,
            );
    ...
    setSnapshotMeta() {
        setTimeout(() => {
            ...
            this.interCom.responsePut({
                action: 'SnapshotMeta',
                payload: {
                    createdBy: dbstate.loadedDB.createdBy,
                    sourceName: dbstate.loadedDB.sourceName,
                    sourceId: dbstate.loadedDB.sourceId,
                    source: dbstate.loadedDB.sourceType,
                    createdTime: this.dateUtil.timestampToTime(
                        (dbstate.loadedDB.createdTime / 1000).toString(),
                        this.dbTime.zone,
                    ),
                },
            });
    ...
    applyDBDownsample(dsample: any) {
        ...
        const cloneWidgets = this.utilService.deepClone(
            this.snapshot ? [this.newWidget] : this.widgets,
        );
    ...
    applyTplVarValue(tvars: any[]) {
        ...
        const tplVars = this.variablePanelMode.view
            ? this.tplVariables.viewTplVariables.tvars
            : this.tplVariables.editTplVariables.tvars;
        ...
        for (let i = 0; i < this.widgets.length; i++) {
            ...
            if (
                this.widgets[i].settings.component_type ===
                    'EventsWidgetComponent' ||
                this.widgets[i].settings.component_type ===
                    'LinechartWidgetComponent'
            ) {
                const search = this.widgets[i].eventQueries
                    ? this.widgets[i].eventQueries[0].search
                    : '';
                if (
                    this.widgets[i].eventQueries &&
                    this.widgets[i].eventQueries[0].namespace &&
                    search !== this.applyTplVarsToEventQuery(search)
                ) {
                    this.handleEventQueryPayload({
                        id: this.widgets[i].id,
                        payload: this.utilService.deepClone({
                            eventQueries: this.widgets[i].eventQueries,
                        }),
                    });
                }
                if (
    ...
    removeCustomTagFilter(payload: any) {
        ...
        for (let i = 0; i < this.widgets.length; i++) {
            ...
            for (let j = 0; j < widget.queries.length; j++) {
                ...
                if (fIndex > -1) {
                    if (
                        filters[fIndex].customFilter &&
                        filters[fIndex].customFilter.length > 0
                    ) {
                        const cFilterIndex = filters[
                            fIndex
                        ].customFilter.indexOf('[' + vartag.alias + ']');
                        if (cFilterIndex > -1) {
                            filters[fIndex].customFilter.splice(
                                cFilterIndex,
                                1,
                            );
                            ...
                            if (
                                filters[fIndex].filter.length === 0 &&
                                filters[fIndex].customFilter.length === 0
                            ) {
        ...
        if (vartag.filter.trim() !== '') {
            ...
            this.store.dispatch(
                new UpdateWidgets(this.utilService.deepClone(this.widgets)),
            );
        }
    }
            ...
            if (loaded) {
                ...
                for (let i = 0; i < this.widgets.length; i++) {
                    ...
                    if (payload.vartag && payload.insert === 1) {
                        isModify = this.dbService.insertTplAliasToWidget(
                            widget,
                            payload,
                            this.dashboardTags.rawDbTags,
                        );
                    } else {
                        isModify = this.dbService.updateTplAliasToWidget(
                            widget,
                            payload,
                        );
                    }
                ...
                if (affectedWidgets.length > 0) {
                    if (payload.insert === 1) {
                        for (
                            let i = 0;
                            i < this.tplVariables.editTplVariables.tvars.length;
                            i++
                        ) {
                            const tvar =
                                this.tplVariables.editTplVariables.tvars[i];
                            tvar.isNew = 0;
    ...
    applyTplToNewWidget(widget: any, tplVars: any[]) {
        const _widget = this.utilService.deepClone(widget);
        // set to false to force get dashboard tags for all widgets nect time
        this.isDbTagsLoaded = false;
        this.httpService
            .getTagKeysForQueries([_widget])
            .subscribe((res: any) => {
                ...
                for (let i = 0; i < tplVars.length; i++) {
                    ...
                    let applied = 0;
                    if (tvar.mode === 'auto') {
                        const isModify = this.dbService.insertTplAliasToWidget(
                            _widget,
                            { vartag: tvar, insert: 1 },
                            widgetTags.rawDbTags,
                        );
                        if (isModify) {
                            anyModify = true;
                            applied = applied + 1;
                            const tplIndex =
                                this.tplVariables.editTplVariables.tvars.findIndex(
                                    (v) => v.alias === tvar.alias,
                                );
                            if (tplIndex > -1) {
                                this.tplVariables.editTplVariables.tvars[
                                    tplIndex
                ...
                if (anyModify) {
                    this.store.dispatch(
                        new UpdateVariables(this.tplVariables.editTplVariables),
                    );
            ...
            for (let j = 0; j < widget.queries.length; j++) {
                ...
                const macthVars = query.filters.filter((f) => {
                    if (
                        ...
                        f.tagk === tvar.tagk &&
                        f.customFilter &&
                        f.customFilter.includes('[' + tvar.alias + ']')
                    ) {
                        return true;
                    } else {
                        return false;
    ...
    updateTplVariablesAppliedCount(payload: any) {
        const idx = this.tplVariables.editTplVariables.tvars.findIndex(
            (tpl) => '[' + tpl.alias + ']' === payload.alias,
        );
        if (idx > -1) {
    ...
    getDashboardTagKeys(reloadData: boolean = true) {
        this.isDbTagsLoaded = false;
        this.httpService.getTagKeysForQueries(this.widgets).subscribe(
            (res: any) => {
                this.dashboardTags =
                    this.dbService.formatDbTagKeysByWidgets(res);
    ...
    changeVarPanelMode(mode: any) {
        if (!mode.view) {
            if (this.tagKeysByNamespaces.length === 0) {
                this.getTagkeysByNamespaces(
                    this.tplVariables.editTplVariables.namespaces,
                );
            }
            if (this.batchToggle) {
    ...
    getQuery(message: any) {
        ...
        const groupby =
            payload.settings.multigraph && payload.settings.multigraph.enabled
                ? payload.settings.multigraph.chart
                    .filter(
                        (d) =>
                            d.key !== 'metric_group' &&
                              d.key !== 'query_group',
                    ...
                    .map((d) => d.key)
                : [];
        const overrideTime = this.isDBZoomed
            ? payload.settings.time.zoomTime
            : this.canWidgetOverrideTime()
                ? payload.settings.time.overrideTime
                : null;

        const dt = overrideTime
            ? this.getDateRange({ ...this.dbTime, ...overrideTime })
            : this.getDashboardDateRange();
        if (this.viewEditMode || this.snapshot) {
            this.editViewModeMeta['queryDataRange'] = {
                start: dt.start / 1000,
                end: dt.end / 1000,
            };
        } else {
            this.wdMetaData[message.id] = {
                queryDataRange: { start: dt.start / 1000, end: dt.end / 1000 },
            };
        }
        if (payload.queries.length) {
            ...
            if (
                this.isDBZoomed &&
                message.id.indexOf('__EDIT__') === -1 &&
                (wType === 'HeatmapWidgetComponent' ||
                    wType === 'LinechartWidgetComponent')
            ) {
            ...
            for (let i = 0; i < payload.queries.length; i++) {
                query = JSON.parse(JSON.stringify(payload.queries[i]));
                groupid = query.id;
                if (query.settings.visual.visible && query.metrics.length) {
                    ...
                    if (tplVars.length > 0) {
                        if (
                            query.filters.findIndex(
                                (f) => f.customFilter !== undefined,
                            ) > -1
                        ) {
                            query = this.dbService.resolveTplVarReplace(
                                query,
                                tplVars,
                                this.tplVariables.scopeCache,
                            );
                        }
                    }
                    ...
                    for (let j = 0; j < query.metrics.length; j++) {
                        const metricGroupBy =
                            query.metrics[j].groupByTags || [];
                        query.metrics[j].groupByTags =
                            this.utilService.arrayUnique(
                                metricGroupBy.concat(groupby),
                            );
            ...
            if (Object.keys(queries).length && sources.length) {
                const tot =
                    ...
                    this.dbToT.value
                        ? this.dbToT
                        : '';
                query = this.queryService.buildQuery(payload, dt, queries, {
                    sources: sources,
                    tot: tot,
                });
            }
    ...
    handleQueryPayload(message: any) {
        if (message.payload.queries.length) {
            ...
            this.store.dispatch(
                new ClearQueryData({ wid: message.id, triggerChange: true }),
            );
        }
    }
    ...
    handleEventQueryPayload(message: any) {
        if (message.payload.eventQueries[0].namespace) {
            const overrideTime =
                this.canWidgetOverrideTime() && message.payload.settings
                    ? message.payload.settings.time.overrideTime
                    : null;
            const dt = overrideTime
                ? this.getDateRange({ ...this.dbTime, ...overrideTime })
                : this.getDashboardDateRange();
            message.payload.eventQueries[0].search =
                this.applyTplVarsToEventQuery(
                    message.payload.eventQueries[0].search,
                );
            this.store.dispatch(
                new GetEvents(
    ...
    getDateRange(range) {
        const startTime = this.dateUtil.timeToMoment(
            range.start.toString(),
            range.zone,
        );
        const endTime = this.dateUtil.timeToMoment(
            range.end.toString(),
            range.zone,
        );
    ...
    widgetsLayoutUpdate(gridLayout: any) {
        ...
        if (gridLayout.clientSize) {
            this.store.dispatch(
                new UpdateGridsterUnitSize(gridLayout.clientSize),
            );
        }
    ...
    addNewWidget(selectedWidget: any) {
        ...
        this.newWidget = this.dbService.getWidgetPrototype(
            selectedWidget.type,
            this.widgets,
        );
    }
            ...
            case 'SetDBDownsample':
                if (this.viewEditMode) {
                    this.dbDownsample = {
                        ...
                        customUnit:
                            message.payload.downsample === 'custom'
                                ? message.payload.customDownsampleUnit
                                : '',
                        customValue:
                            message.payload.downsample === 'custom'
                                ? message.payload.customDownsampleValue
                                : '',
                    };
                    this.refresh();
                } else {
    ...
    setTimezone(e: any) {
        ...
        if (this.isDBZoomed) {
            this.dbTime.start = this.dateUtil.timestampToTime(
                this.dateUtil
                    .timeToMoment(this.dbTime.start, this.dbTime.zone)
                    .unix()
                    .toString(),
                e,
            );
            this.dbTime.end = this.dateUtil.timestampToTime(
                this.dateUtil
                    .timeToMoment(this.dbTime.end, this.dbTime.zone)
                    .unix()
                    .toString(),
    ...
    getOwnerFromPath(fullPath: string) {
        ...
        if (fullPath && fullPath.length) {
            ...
            if (split.length >= 3 && split[0] === '') {
                return (
                    '/' + split[1].toLowerCase() + '/' + split[2].toLowerCase()
                );
    ...
    notifyWidgetLoaderUserHasWriteAccess(
        userHasWriteAccessToNamespace: boolean,
    ) {
        this.interCom.requestSend({
    ...
    openDashboardDeleteDialog() {
        ...
        this.dashboardDeleteDialog = this.dialog.open(
            DashboardDeleteDialogComponent,
            dialogConf,
        );
        this.dashboardDeleteDialog
            .afterClosed()
    ...
    createAlertFromWidget(message) {
        ...
        if (namespaces.length === 1) {
            this.dataShare.setData({
                widgetId: message.id,
                widget: message.payload,
                dashboardId: this.dbid,
                namespace: namespaces[0],
                tplVariables: this.tplVariables.viewTplVariables,
            });
            ...
            this.dashboardToAlertDialog = this.dialog.open(
                DashboardToAlertDialogComponent,
                dialogConf,
            );
            this.dashboardToAlertDialog
                .afterClosed()
                .subscribe((dialog_out: any) => {
                    if (dialog_out && dialog_out.namespace) {
                        this.dataShare.setData({
                            ...
                            widget: message.payload,
                            dashboardId: this.dbid,
                            namespace: dialog_out.namespace,
                            tplVariables: this.tplVariables.viewTplVariables,
                        });
                        this.dataShare.setMessage('WidgetToAlert');
                        this.router.navigate([
                            'a',
                            dialog_out.namespace,
                            '_new_',
                        ]);
                    }
                });
    ...
    buildDownloadQueryHelpString() {
        ...
        const examples =
            this.appConfig.getConfig().widget.downloadQuery.examples;
    ...
    downloadDataQuery(message) {
        ...
        const data =
            this.buildDownloadQueryHelpString() +
            'QUERY:\n' +
    ...
    dashboardFavoriteAction(remove?: boolean) {
        if (remove) {
            ...
            const data: any = this.store.selectSnapshot(
                DbfsResourcesState.getFileById(this.dbid),
            );
            this.store.dispatch(
                new DbfsRemoveUserFav(data, {
    ...
    toggleBatchControls() {
        ...
        if (!this.batchToggle) {
            const keys = Object.keys(this.batchSelectedItems);
            const selectedItems = {};
            this.batchSelectAll = false;
            this.batchSelectAllIndeterminate = false;
    ...
    batchCopyToClipboard() {
        ...
        setTimeout(() => {
            ...
            const widgets: any[] = this.getBatchSelectedItems();

            const widgetLoaders = this.dbContent.widgetLoaders;

            const promises: any[] = [];

            // we have to get the widgetLoaders so we can locate the visualization element to take a snapshot image
            for (let i = 0; i < widgets.length; i++) {
                const loader = widgetLoaders.find((item: any) => item.widget.id === widgets[i].id);
                ...
                // create preview
                const componentEl: any =
                    loader._component.instance.elRef.nativeElement;
                componentEl.style.backgroundColor = '#ffffff';
                const p = domtoimage.toJpeg(componentEl).then((dataUrl: any) => {
            ...
            Promise.all(promises)
                .then((results) => {
                    ...
                    const resolvedWidgets: any[] =
                        this.resolveDbTplVariablesForClipboard(results);
                ...
                .catch((e) => {
                    ...
                    this.batchSelectedCount = this.batchSelectAll
                        ? keys.length
                        : 0;
                    this.batchSelectedItems = selectedItems;
                });
    ...
    openBatchDeleteDialog() {
        ...
        this.batchWidgetDeleteDialog = this.dialog.open(
            WidgetDeleteDialogComponent,
            dialogConf,
        );
        this.batchWidgetDeleteDialog
            .afterClosed()
            .subscribe((dialog_out: any) => {
                if (dialog_out && dialog_out.delete) {
    ...
    batchRemoveWidgets() {
        const ids: string[] = this.getBatchSelectedIds();
    ...
    private getBatchSelectedIds(): any[] {
        const keys = Object.keys(this.batchSelectedItems);
        const selected = keys.filter((item) => this.batchSelectedItems[item] === true);
        return selected;
    }

    // get the selected item(s) and return array of widgets selected
    private getBatchSelectedItems(): any[] {
        const ids = this.getBatchSelectedIds();

        const items = this.widgets.filter((item: any) => ids.includes(item.id));
        return items;
    }
    ...
    private resolveDbTplVariablesForClipboard(widgets: any[]): any[] {
        const dbTplVarLookup = this.getTplVariablesKeyLookup();
        ...
        for (let i = 0; i < widgets.length; i++) {
            ...
            for (let q = 0; q < widget.queries.length; q++) {
                const query: any = widget.queries[q];

                // loop through filters
                for (let f = 0; f < query.filters.length; f++) {
                    const filter: any = query.filters[f];
                    ...
                    if (filter.customFilter && filter.customFilter.length > 0) {
                        ...
                        const fval = dbTplVarLookup[fkey].filter
                            ? dbTplVarLookup[fkey].filter
                            : undefined;
                        const fvalScope = dbTplVarLookup[fkey].scope
                            ? dbTplVarLookup[fkey].scope
                            : undefined;

                        filter.customFilter = [];

                        let filterValue: any[] = Array.isArray(filter.filter)
                            ? filter.filter
                            : [];
