...
export class DboardContentComponent implements OnChanges {
    ...
    @ViewChild(WidgetViewDirective, { static: true })
    widgetViewContainer: WidgetViewDirective;
    @ViewChild(GridsterComponent) gridster: GridsterComponent;

    // widgetLoader Children
    @ViewChildren(WidgetLoaderComponent, { read: WidgetLoaderComponent })
    widgetLoaders: QueryList<WidgetLoaderComponent>;
    ...
    @Input() dbid: any;
    @Input() readonly = true;
    @Input() batchControlsToggle = false;
    ...
    ngOnChanges(changes: SimpleChanges) {
        ...
        if (
            changes.rerender &&
            changes.rerender.currentValue.reload &&
            !this.viewEditMode
        ) {
        ...
        if (changes.dashboardMode) {
            this.viewEditMode =
                changes.dashboardMode.currentValue === 'edit' ||
                changes.dashboardMode.currentValue === 'snap' ||
                changes.dashboardMode.currentValue === 'explore';
            if (
                !this.viewEditMode &&
                this.widgetViewContainer.viewContainerRef
            ) {
        ...
        if (!override && this.dashboardMode !== 'snap') {
            ...
            widget.settings = {
                ...
                ...this.widgetService.getWidgetDefaultSettings(
                    widget.settings.component_type,
                ),
            };
        }
        const component: Type<any> = this.widgetService.getComponentToLoad(
            widget.settings.component_type,
        );
    ...
    loadComponent(comp: any) {
        ...
        const editWidget =
            mode !== 'snap'
                ? JSON.parse(JSON.stringify(comp.widget))
                : comp.widget;
        editWidget.id =
            mode !== 'snap' ? '__EDIT__' + comp.widget.id : comp.widget.id;
        // assign @input widget
    ...
    gridEventEnd(event: any) {
        ...
        if (event.action === 'resize' || event.action === 'drag') {
            ...
            this.widgetsLayoutUpdate.emit(
                this.getWigetPosition(width, height, this.winSize),
            );
