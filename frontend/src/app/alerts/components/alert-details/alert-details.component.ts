...
} from './children/alert-details-metric-period-over-period/alert-details-metric-period-over-period.component';
...
implements OnInit, OnDestroy, AfterContentInit, AfterViewInit {
    ...
    @ViewChild('alertDateTimeNavbarItemTmpl', { static: true })
    alertDateTimeNavbarItemTmpl: TemplateRef<any>;
    @ViewChild('periodOverPeriodForm')
    periodOverPeriodForm: AlertDetailsMetricPeriodOverPeriodComponent;

    @Input() response;

    @Input() viewMode = ''; // edit || clone || view
    ...
    get readOnly(): boolean {
        ...
        return this.viewMode === 'edit' || this.viewMode === 'clone'
            ? false
            : true;
    }
    ...
    slidingWindowToDSTimeRange(seconds) {
        const validTimeRanges = [
            60, 300, 600, 900, 1800, 3600, 21600, 43200, 86400, 172800,
        ];
    ...
    metricTableDisplayColumns: string[] = ['metric-index', 'name', 'modifiers'];

    excludeMetricGroupByTags = [
        '_aggregate',
        '_alert_id',
        '_alert_name',
        '_threshold_name',
    ...
    ngOnInit() {
        ...
        this.subscription.add(
            this.interCom.responseGet().subscribe((message: any) => {
        ...
        if (this.data.id) {
            this.data.createdTime = this.dateUtil.timestampToTime(
                (this.data.createdTime / 1000).toString(),
                'local',
            );
            this.data.updatedTime = this.dateUtil.timestampToTime(
                (this.data.updatedTime / 1000).toString(),
                'local',
            );
        }
        ...
        if (this.dataShare.getMessage() === 'WidgetToAlert') {
            ...
            this.createdFrom = {
                widgetId: this.dataShare.getData().widgetId,
                dashboardId: this.dataShare.getData().dashboardId,
            };
    ...
    ngAfterContentInit() {
        ...
        if (this.graphOutput && this.data.id > 0 && window.innerHeight) {
            this.graphOutput.nativeElement.style.height =
                window.innerHeight * 0.4 + 'px';
        }
        const resizeSensor = new ResizeSensor(
            this.graphOutput.nativeElement,
            (size) => {
                const newSize = {
                    width:
                        size.width * (this.data.type === 'event' ? 0.65 : 1) -
                        5,
                    height: size.height,
                };
                this.size = newSize;
    ...
    newSingleMetricTimeWindowSelected(timeInSeconds: string) {
        this.alertForm.controls['threshold']['controls']['singleMetric'][
            'controls'
        ]['slidingWindow'].setValue(timeInSeconds);
    ...
    setSingleMetricReportingInterval(timeInSeconds: string) {
        this.alertForm.controls['threshold']['controls']['singleMetric'][
            'controls'
        ]['reportingInterval'].setValue(timeInSeconds);
    ...
    periodOverPeriodChanged(periodOverPeriodConfig) {
        if (periodOverPeriodConfig.thresholdChanged) {
            this.determineEnabledTransitions(
                periodOverPeriodConfig.config.periodOverPeriod,
            );
        }
    ...
    determineEnabledTransitions(config) {
        ...
        if (upperBad || lowerBad) {
            this.utils.addTransitions(this.periodOverPeriodTransitionsEnabled, [
                'BadToGood',
                'GoodToBad',
            ]);
        }
        ...
        this.periodOverPeriodTransitionsSelected = [
            ...this.periodOverPeriodTransitionsEnabled,
        ];
    ...
    setupForm(data = null) {
        if (
            data &&
            data.threshold &&
            data.threshold.subType === 'periodOverPeriod'
        ) {
            this.determineEnabledTransitions(data.threshold.periodOverPeriod);
            this.periodOverPeriodTransitionsSelected = [
                ...data.notification.transitionsToNotify,
            ];
        ...
        const bad =
            data.threshold.singleMetric.badThreshold !== undefined
                ? data.threshold.singleMetric.badThreshold
                : null;
        const warn =
            data.threshold.singleMetric.warnThreshold !== undefined
                ? data.threshold.singleMetric.warnThreshold
                : null;
        const recover =
            data.threshold.singleMetric.recoveryThreshold !== undefined
                ? data.threshold.singleMetric.recoveryThreshold
                : null;
        const notifyOnMissing = data.threshold.notifyOnMissing
            ? data.threshold.notifyOnMissing.toString()
            : 'false';
        // eslint-disable-next-line max-len
        const metricId = data.threshold.singleMetric.metricId
            ? this.utils.getMetricDropdownValue(
                data.queries.raw,
                data.threshold.singleMetric.metricId,
            )
            : '';
        const [qindex, mindex] = metricId
            ? this.utils.getMetricIndexFromId(metricId, this.queries)
            : [null, null];
        this.alertForm = this.fb.group({
            ...
            threshold: this.fb.group(
                {
                    ...
                    autoRecoveryInterval:
                        data.threshold.autoRecoveryInterval || 'null',
                    delayEvaluation: data.threshold.delayEvaluation || 0,
                    singleMetric: this.fb.group({
                        queryIndex:
                            data.threshold.singleMetric.queryIndex || -1,
                        queryType:
                            data.threshold.singleMetric.queryType || 'tsdb',
                        // eslint-disable-next-line max-len
                        metricId: [metricId],
                        badThreshold: bad,
                        warnThreshold: warn,
                        requiresFullWindow:
                            data.threshold.singleMetric.requiresFullWindow ||
                            false,
                        reportingInterval:
                            data.threshold.singleMetric.reportingInterval ||
                            null,
                        ...
                        slidingWindow: data.threshold.singleMetric.slidingWindow
                            ? data.threshold.singleMetric.slidingWindow.toString()
                            : this.defaultSlidingWindowSize,
                        comparisonOperator:
                            data.threshold.singleMetric.comparisonOperator ||
                            'above',
                        timeSampler:
                            data.threshold.singleMetric.timeSampler ||
                            'at_least_once',
                    }),
                },
            ...
            notification: this.fb.group({
                ...
                opsgeniePriority:
                    data.notification.opsgeniePriority ||
                    this.defaultOpsGeniePriority,
                opsgenieAutoClose: data.notification.opsgenieAutoClose || false,
                opsgenieTags: this.fb.array(
                    data.notification.opsgenieTags || [],
                ),
                // OC conditional values
                runbookId: data.notification.runbookId || '',
                ocSeverity:
                    data.notification.ocSeverity || this.defaultOCSeverity,
                ocTier: data.notification.ocTier || this.defaultOCTier,
                // PagerDuty conditional values
                pagerdutyAutoClose:
                    data.notification.pagerdutyAutoClose || false,
            }),
        });
        this.prevTimeSampler =
            data.threshold.singleMetric.timeSampler || 'at_least_once';
        this.setTags();
        this.reloadData();
        ...
        this.subscription.add(
            <Subscription>this.alertForm.controls['threshold']['controls'][
                'singleMetric'
            ]['controls']['comparisonOperator'].valueChanges.subscribe(
                (val) => {
                    this.thresholdSingleMetricControls[
                        'warnThreshold'
                    ].setErrors(null);
                    this.thresholdSingleMetricControls[
                        'recoveryThreshold'
        ...
        this.subscription.add(
            <Subscription>(
                this.alertForm.controls['threshold']['controls'][
                    'singleMetric'
                ]['controls']['badThreshold'].valueChanges
                    .pipe(
                        startWith(
                            this.alertForm.controls['threshold']['controls'][
                                'singleMetric'
                            ]['controls']['badThreshold'].value,
                        ),
                        pairwise(),
                    )
                    .subscribe(([prev, tbad]: [any, any]) => {
                        this.setThresholds('bad', tbad);
                        ...
                        const transitions = this.alertForm[
                            'controls'
                        ].notification.get('transitionsToNotify').value;
                        if (tbad === null) {
                            // remove possible transitions (if any were selected)
                            this.alertForm['controls'].notification
                                .get('transitionsToNotify')
                                .setValue(
                                    transitions.filter(
                                        (d) => !possibleTransitions.includes(d),
                                    ),
                                );
                        } else if (prev === null && tbad !== null) {
                            // In case warn threshold is empty, do not check warn/bad combos
                            if (
                                this.alertForm.controls['threshold'][
                                    'controls'
                                ]['singleMetric']['controls']['warnThreshold']
                                    .value === null
                            ) {
                                possibleTransitions =
                                    possibleTransitions.filter(
                                        (d) =>
                                            !d.toLowerCase().includes('warn'),
                                    );
                            }
                            ...
                            this.alertForm['controls'].notification
                                .get('transitionsToNotify')
                                .setValue(
                                    transitions.concat(possibleTransitions),
                                );
                        }
                        this.thresholdSingleMetricControls[
                            'warnThreshold'
        ...
        this.subscription.add(
            <Subscription>(
                this.alertForm.controls['threshold']['controls'][
                    'singleMetric'
                ]['controls']['warnThreshold'].valueChanges
                    .pipe(
                        startWith(
                            this.alertForm.controls['threshold']['controls'][
                                'singleMetric'
                            ]['controls']['warnThreshold'].value,
                        ),
                        pairwise(),
                    )
                    .subscribe(([prev, twarn]: [any, any]) => {
                        this.setThresholds('warning', twarn);
                        let possibleTransitions = [
                            'warnToBad',
                            'badToWarn',
                            'warnToGood',
                        ...
                        } else if (prev === null && twarn !== null) {
                            // In case bad threshold is empty, do not check warn/bad combos
                            if (
                                this.alertForm.controls['threshold'][
                                    'controls'
                                ]['singleMetric']['controls']['badThreshold']
                                    .value === null
                            ) {
                                possibleTransitions =
                                    possibleTransitions.filter(
                                        (d) => !d.toLowerCase().includes('bad'),
                                    );
                            }
                            // if it was previously empty/null, then turn on the default transitions
                            this.alertForm['controls'].notification
        ...
        this.subscription.add(
            <Subscription>this.alertForm.controls['threshold']['controls'][
                'singleMetric'
            ]['controls']['recoveryThreshold'].valueChanges.subscribe((val) => {
                const recoveryType =
                    this.alertForm.controls['threshold']['controls'][
                        'singleMetric'
                    ]['controls']['recoveryType'].value;
                if (val !== null && recoveryType === 'minimum') {
                    this.thresholdSingleMetricControls['recoveryType'].setValue(
                        'specific',
                    );
                }
        ...
        this.subscription.add(
            <Subscription>this.alertForm.controls['threshold']['controls'][
                'singleMetric'
            ]['controls']['timeSampler'].valueChanges.subscribe((val) => {
                if (
                    (['at_least_once', 'all_of_the_times'].includes(
                        this.prevTimeSampler,
                    ) &&
                        ['on_avg', 'in_total'].includes(val)) ||
                    (['at_least_once', 'all_of_the_times'].includes(val) &&
                        ['on_avg', 'in_total'].includes(
                            this.prevTimeSampler,
                        )) ||
                    (['on_avg', 'in_total'].includes(this.prevTimeSampler) &&
                        ['on_avg', 'in_total'].includes(val))
                ) {
                    this.getData();
                }
        ...
        this.subscription.add(
            <Subscription>this.alertForm.controls['threshold']['controls'][
                'singleMetric'
            ]['controls']['recoveryType'].valueChanges.subscribe((val) => {
                this.thresholdSingleMetricControls[
                    'recoveryThreshold'
                ].setErrors(null);
                // eslint-disable-next-line max-len
                this.setThresholds(
                    ...
                    val === 'specific'
                        ? this.alertForm.controls['threshold']['controls'][
                            'singleMetric'
                        ]['controls']['recoveryThreshold'].value
                        : '',
                );
            }),
        );
        ...
        this.subscription.add(
            <Subscription>this.alertForm.controls['threshold']['controls'][
                'singleMetric'
            ]['controls']['metricId'].valueChanges.subscribe((val) => {
        ...
        this.alertForm['controls'].threshold
            .get('notifyOnMissing')
            .setValue(notifyOnMissing, { emitEvent: true });
    ...
    metricIdChanged(mid) {
        const [qindex, mindex] = mid
            ? this.utils.getMetricIndexFromId(mid, this.queries)
            : [null, null];
        ...
        this.setTags();
        const namespace = this.queries[qindex]
    ...
    setupHealthCheckForm(data = null) {
        ...
        this.showDetail = data.id ? true : false;
        this.showThresholdAdvanced =
            data.threshold.nagInterval > 0 ? true : false;
        this.setQuery();
        this.alertForm = this.fb.group({
            name: data.name || 'Untitled Alert',
            type: 'healthcheck',
            ...
            threshold: this.fb.group({
                ...
                nagInterval: data.threshold.nagInterval || '0',
                notifyOnMissing: data.threshold.notifyOnMissing
                    ? data.threshold.notifyOnMissing.toString()
                    : 'false',
                missingDataPurgeInterval:
                    data.threshold.missingDataPurgeInterval || null,
                missingDataInterval: data.threshold.missingDataInterval || null,
                healthCheck: this.fb.group({
                    queryIndex: 0,
                    queryType: 'aura',
                    badThreshold:
                        data.threshold.healthCheck.badThreshold || null,
                    recoveryThreshold:
                        data.threshold.healthCheck.recoveryThreshold || 1,
                    warnThreshold:
                        data.threshold.healthCheck.warnThreshold || null,
                    unknownThreshold:
                        data.threshold.healthCheck.unknownThreshold || null,
                }),
            }),
            notification: this.fb.group({
                transitionsToNotify: [
        ...
        this.subscription.add(
            <Subscription>this.alertForm.controls['threshold']['controls'][
                'healthCheck'
            ]['controls']['badThreshold'].valueChanges.subscribe((bad) => {
                const transitions = this.alertForm['controls'].notification.get(
                    'transitionsToNotify',
                ).value;
                if (!bad) {
                    // remove possible transitions (if any were selected)
        ...
        this.subscription.add(
            <Subscription>this.alertForm.controls['threshold']['controls'][
                'healthCheck'
            ]['controls']['warnThreshold'].valueChanges.subscribe((warn) => {
                const transitions = this.alertForm['controls'].notification.get(
                    'transitionsToNotify',
                ).value;
                if (!warn) {
        ...
        this.subscription.add(
            <Subscription>this.alertForm.controls['threshold']['controls'][
                'healthCheck'
            ]['controls']['unknownThreshold'].valueChanges.subscribe(
                (unknown) => {
                    const transitions = this.alertForm[
                        'controls'
                    ].notification.get('transitionsToNotify').value;
                    if (!unknown) {
                        this.alertForm['controls'].notification
                            ...
                            .setValue(
                                transitions.filter(
                                    (d) => !d.toLowerCase().includes('unknown'),
                                ),
                            );
                    }
                },
    ...
    newEventTimeWindowSelected(timeInSeconds: string) {
        this.alertForm.controls['threshold']['controls']['eventAlert'][
            'controls'
        ]['slidingWindow'].setValue(timeInSeconds);
    }
    ...
    setupEventForm(data = null) {
        ...
        this.alertForm = this.fb.group({
            ...
            threshold: this.fb.group({
                ...
                eventAlert: this.fb.group({
                    ...
                    slidingWindow:
                        data.threshold.eventAlert.slidingWindow ||
                        this.defaultEventSlidingWindowSize,
                }),
            }),
            notification: this.fb.group({
                transitionsToNotify: [
                    data.notification.transitionsToNotify || ['goodToBad'],
                ],
                recipients: [data.notification.recipients || {}],
                subject: data.notification.subject || '',
                body: data.notification.body || '',
        ...
        this.startTime =
            this.dsTimeRange[
                this.slidingWindowToDSTimeRange(
                    this.alertForm
                        .get('threshold')
                        .get('eventAlert')
                        .get('slidingWindow').value,
                )
            ].label;
        this.endTime = 'now';
        ...
        this.alertForm
            .get('queries')
            .get('eventdb')
            ['controls'][0].valueChanges.subscribe((changes) => {
                this.doEventQuery$.next(['list', 'count']);
            });
        this.alertForm
            .get('threshold')
    ...
    setQuery() {
        if (
            this.data.threshold &&
            this.data.threshold.suppress &&
            this.data.threshold.suppress.metricId
        ) {
            this.suppressConfig.metricId = this.data.threshold.suppress.metricId
                ? this.utils.getMetricDropdownValue(
                    this.data.queries.raw,
                    this.data.threshold.suppress.metricId,
                )
                : '';
            this.suppressConfig.reportingInterval =
                this.data.threshold.suppress.reportingInterval || 60;
            this.suppressConfig.comparisonOperator =
                this.data.threshold.suppress.comparisonOperator || 'missing';
            this.suppressConfig.timeSampler =
                this.data.threshold.suppress.timeSampler || 'all_of_the_times';
            this.suppressConfig.threshold =
                this.data.threshold.suppress.threshold || 0;
            this.suppressConfig = { ...this.suppressConfig };
            this.suppressConfig.disabled = false;
        }
        this.queries =
            this.data.queries && this.data.queries.raw
                ? this.data.queries.raw
                : [this.getNewQueryConfig()];
    }
    ...
    deleteQuery(qid) {
        const [qidx, midx] = this.utils.getMetricIndexFromId(
            this.thresholdSingleMetricControls.metricId.value,
            this.queries,
        );
        const qindex = this.queries.findIndex((d) => d.id === qid);
        if (qidx === qindex) {
    ...
    deleteQueryMetric(qid, mid) {
        ...
        if (this.queries[qindex]) {
            const mindex = this.queries[qindex].metrics.findIndex(
                (d) => d.id === mid,
            );
    ...
    doEventQuery(list) {
        const id = 'q1_m1';
        let namespace = this.alertForm
            .get('queries')
            .get('eventdb')
            ['controls'][0].get('namespace').value;
        namespace = namespace !== null ? namespace.trim() : '';
        const filter = this.alertForm
            .get('queries')
            .get('eventdb')
            ['controls'][0].get('filter').value;
        const downsample = this.alertForm
            .get('threshold')
            .get('eventAlert')
            .get('slidingWindow').value;
        const query = {
        ...
        if (list.includes('count') && namespace) {
            ...
            eventQueries[1].downSample = {
                aggregator: 'sum',
                value: downsample / 60 + 'm',
            };
        }
        const time = {
            start: this.dateUtil
                .timeToMoment(this.startTime, 'local')
                .valueOf(),
        ...
        if (eventQueries.length) {
            ...
            this.error = '';
            this.httpService
                .getEvents('123', time, eventQueries, 100)
                .subscribe(
                    (res) => {
                        ...
                        const data = this.dataTransformer.openTSDBToDygraph(
                            config,
                            this.options,
                            [[0]],
                            res.counts,
                        ...
                        let max = 0,
                            min = Infinity;
                        if (
                            res.counts.results.length &&
                            res.counts.results[0].data.length
                        ) {
                            for (
                                let i = 0;
                                i <
                                res.counts.results[0].data[0].NumericType
                                    .length -
                                    1;
                                i++
                            ) {
    ...
    setTimezone(e: any) {
        this.startTime = this.dateUtil.isRelativeTime(this.startTime)
            ? this.startTime
        ...
        this.endTime = this.dateUtil.isRelativeTime(this.endTime)
            ? this.endTime
            : this.dateUtil.timestampToTime(
                this.dateUtil
    ...
    validateSingleMetricThresholds(group) {
        const slidingWindowCntrl =
            this.thresholdSingleMetricControls['slidingWindow'];
        const badStateCntrl =
            this.thresholdSingleMetricControls['badThreshold'];
        const warningStateCntrl =
            this.thresholdSingleMetricControls['warnThreshold'];
        const recoveryStateCntrl =
            this.thresholdSingleMetricControls['recoveryThreshold'];
        const reportingIntervalCntrl =
            this.thresholdSingleMetricControls['reportingInterval'];
        const requiresFullWindowCntrl =
            this.thresholdSingleMetricControls['requiresFullWindow'];
        const autoRecoveryIntervalCntrl =
            this.thresholdControls['autoRecoveryInterval'];
        ...
        const operator = this.alertForm
            .get('threshold')
            .get('singleMetric')
            .get('comparisonOperator').value;
        const timeSampler =
            this.thresholdSingleMetricControls['timeSampler'].value;

        if (
            this.alertForm.touched &&
            badStateCntrl.value === null &&
            warningStateCntrl.value === null
        ) {
        ...
        if (
            ...
            reportingIntervalCntrl.value !== null &&
            parseInt(reportingIntervalCntrl.value) >
                parseInt(slidingWindowCntrl.value)
        ) {
        ...
        if (
            ...
            if (
                (operator === 'above' || operator === 'above_or_equal_to') &&
                warning >= bad
            ) {
                warningStateCntrl.setErrors({ invalid: true });
            }
            if (
                (operator === 'below' || operator === 'below_or_equal_to') &&
                warning <= bad
            ) {
                warningStateCntrl.setErrors({ invalid: true });
            }
        ...
        if (
            ...
            recoveryStateCntrl.value === null
        ) {
            this.thresholdSingleMetricControls['recoveryThreshold'].setErrors({
                required: true,
            });
        }
        ...
        if (
            ...
            if (
                (operator === 'above' && recovery > badOrWarning) ||
                (operator === 'below' && recovery < badOrWarning) ||
                (operator === 'above_or_equal_to' &&
                    recovery >= badOrWarning) ||
        ...
        if (
            autoRecoveryIntervalCntrl.value !== null &&
            parseInt(autoRecoveryIntervalCntrl.value) <=
                parseInt(slidingWindowCntrl.value)
        ) {
    ...
    validateHealthCheckForm() {
        ...
        const missingDataInterval =
            this.thresholdControls['missingDataInterval'].value;
        ...
        if (notifyOnMissing === 'true' && missingDataInterval === null) {
            this.thresholdControls.missingDataInterval.setErrors({
                required: true,
            });
        }
    ...
    validateEventAlertForm() {
        const namespace = this.alertForm
            .get('queries')
            .get('eventdb')
            ['controls'][0].get('namespace').value;
        const threshold = this.alertForm
            .get('threshold')
            .get('eventAlert')
            .get('threshold').value;
        if (!namespace) {
    ...
    setTags() {
        if (this.thresholdType === 'singleMetric') {
            ...
            if (
                mid &&
                this.queries[qindex] &&
                this.queries[qindex].metrics.length
            ) {
            ...
            if (namespace) {
                this.httpService
                    .getNamespaceTagKeys(query, 'meta')
                    .subscribe((res) => {
            ...
            if (this.queries[0].namespace !== '') {
                this.httpService
                    .getNamespaceTagKeys(query, 'aurastatus')
                    .subscribe((res) => {
                        this.tags = res.map((d) => d.name);
                    });
            }
    ...
    getMetricGroupByTags(qindex, mindex) {
        return this.queries[qindex] && this.queries[qindex].metrics[mindex]
    ...
    get thresholdSingleMetricControls() {
        return this.alertForm['controls'].threshold['controls'].singleMetric[
            'controls'
        ];
    }

    get healthCheckControls() {
        return this.alertForm['controls'].threshold['controls'].healthCheck[
            'controls'
        ];
    }
    ...
    handleZoom(zConfig) {
        ...
        if (zConfig.isZoomed && n > 0 && zConfig.axis === 'x') {
            ...
            const startTime =
                new Date(this.chartData.ts[0][0]).getTime() / 1000;
            const endTime =
                new Date(this.chartData.ts[n - 1][0]).getTime() / 1000;
            this.startTime =
                Math.floor(zConfig.start) <= startTime
                    ? this.startTime
                    : this.dateUtil.timestampToTime(
                        zConfig.start,
                        this.timeZone,
            ...
            this.endTime =
                Math.ceil(zConfig.end) >= endTime
                    ? this.endTime
                    : this.dateUtil.timestampToTime(zConfig.end, this.timeZone);
    ...
    getData() {
        ...
        const timeSampler =
            this.data.threshold &&
            this.data.threshold.subType === 'singleMetric'
                ? this.alertForm.controls['threshold']['controls'][
                    'singleMetric'
                ]['controls']['timeSampler'].value
                : null;
        const slidingWindow =
            this.data.threshold &&
            this.data.threshold.subType === 'singleMetric'
                ? this.alertForm.controls['threshold']['controls'][
                    'singleMetric'
                ]['controls']['slidingWindow'].value
                : '0';
        const fx =
            timeSampler === 'on_avg'
                ? { fxCall: 'EWMA', val: slidingWindow + 's,0.0' }
                : timeSampler === 'in_total'
                    ? {
                        fxCall: 'SlidingWindow',
                        val: 'sum,' + slidingWindow + 's',
                    }
                    : null;
        let i = 0;
        ...
        const options: any = {};
        if (
            Object.keys(this.periodOverPeriodConfig).length &&
            this.data.threshold.subType === 'periodOverPeriod'
        ) {
            options.periodOverPeriod =
                this.periodOverPeriodConfig.periodOverPeriod;
        }
        ...
        options.sources = mid
            ? this.suppressConfig.metricId
                ? [mid, this.suppressConfig.metricId]
                : [mid]
            : [];
        if (Object.keys(queries).length) {
            const query = this.queryService.buildQuery(
                settings,
                this.queryTime,
                queries,
                options,
    ...
    getTsdbQuery(mid) {
        ...
        if (
            ...
            options.periodOverPeriod =
                this.periodOverPeriodConfig.periodOverPeriod;
            settings.settings.time = {};
            settings.settings.time.downsample = {
                aggregator: 'avg',
                value: 'custom',
                customValue: 1,
                customUnit: 'm',
        ...
        const q = this.queryService.buildQuery(
            settings,
            time,
            queries,
            options,
    ...
    getCount() {
        if (this.data && this.data.namespace && this.data.id) {
            const countObserver = this.httpService.getAlertCount({
    ...
    setChartYMax() {
        ...
        switch (this.data.type) {
            case 'simple':
                const bad =
                    this.thresholdSingleMetricControls['badThreshold'].value;
                const warning =
                    this.thresholdSingleMetricControls['warnThreshold'].value;
                const recovery =
                    this.thresholdSingleMetricControls['recoveryThreshold']
                        .value;
        ...
        this.options.axes.y.valueRange[0] =
            min < this.options.axes.y.tickFormat.min ? min - min * 0.1 : null;
        this.options.axes.y.valueRange[1] =
            max && max > this.options.axes.y.tickFormat.max
                ? max + max * 0.1
                : null;
    }
    ...
    updateQuery(message) {
        ...
        switch (message.action) {
            case 'QueryChange':
                ...
                this.showDetail =
                    this.showDetail === false
                        ...
                        : this.showDetail;
                this.setTags();
                if (this.thresholdType !== 'healthCheck') {
                    // no preview data for healthCheck
                    this.reloadData();
                }
            ...
            case 'UpdateQueryMetricVisual':
                const qindex = this.queries.findIndex(
                    (d) => d.id === message.id,
                );
                const mindex = this.queries[qindex].metrics.findIndex(
                ...
                // eslint-disable-next-line max-len
                this.queries[qindex].metrics[mindex].settings.visual = {
                    ...this.queries[qindex].metrics[mindex].settings.visual,
    ...
    timeseriesTickListener(event: any) {
        if (event.action === 'openLegend' && !this.tsLegendOptions.open) {
            ...
            (event.action === 'openLegend' && !this.tsLegendOptions.open) ||
            event.action === 'tickDataChange'
        ) {
            const payload: any = {
    ...
    setSeriesVisibilityConfig(index: number, visibility: boolean) {
        ...
        this.options.visibilityHash[options.series[index + 1].hash] =
            options.visibility[index];
        this.options = { ...this.options };
    }
    ...
    toggleAlert() {
        this.configChange.emit({
            action: 'ToggleAlert',
            payload: { id: this.data.id, enabled: this.enabled },
        });
    }
    ...
    metricSubTypeChanged(e) {
        ...
        if (
            e.value === 'singleMetric' ||
            (e.value === 'periodOverPeriod' &&
                Object.keys(this.periodOverPeriodConfig).length > 0)
        ) {
            this.reloadData();
        }
        ...
        if (
            e.value === 'singleMetric' &&
            this.periodOverPeriodConfig &&
            this.periodOverPeriodConfig.delayEvaluation != null
        ) {
            this.alertForm['controls'].threshold
                .get('delayEvaluation')
                .setValue(this.periodOverPeriodConfig.delayEvaluation);
        }

        if (e.value === 'periodOverPeriod') {
            // singleMetric thresholds can interfere with rendering of periodOverPeriod graph
            ...
            this.alertForm['controls'].threshold['controls'].singleMetric
                .get('warnThreshold')
                .setValue(null);
            this.periodOverPeriodConfig.delayEvaluation =
                this.alertForm['controls'].threshold.get(
                    'delayEvaluation',
                ).value;
        }
    }
    ...
    validate(showTopErrorBar = true) {
        ...
        switch (this.data.type) {
            case 'simple':
                ...
                if (!this.thresholdSingleMetricControls.metricId.value) {
                    this.thresholdSingleMetricControls.metricId.setErrors({
                        required: true,
                    });
                }
                ...
                if (this.data.threshold.subType === 'periodOverPeriod') {
                    ...
                    if (
                        !this.alertForm['controls'].notification.get(
                            'transitionsToNotify',
                        ).value.length
                    ) {
                        this.alertForm['controls'].notification
                    ...
                    if (this.suppressConfig.metricId) {
                        const [qindex, mindex] =
                            this.utils.getMetricIndexFromId(
                                this.suppressConfig.metricId,
                                this.queries,
                            );
                        const suppressTags =
                        ...
                        if (
                            (this.tags.length &&
                                (!suppressTags.length ||
                                    !this.utils.isArraySubset(
                                        this.tags,
                                        suppressTags,
                                    ))) ||
                            (this.suppressConfig.comparisonOperator !==
                                'missing' &&
                                (this.suppressConfig.threshold === null ||
                                    (this.suppressConfig.timeSampler ===
                                        'all_of_the_times' &&
                                        this.suppressConfig.reportingInterval <=
                                            0)))
        ...
        if (
            this.alertForm['controls'].notification.get('body').value.trim() ===
            ''
        ) {
            this.alertForm['controls'].notification
                .get('body')
    ...
    saveAlert() {
        ...
        switch (data.type) {
            case 'simple':
                ...
                let dsId = this.utils.getDSId(
                    this.utils.arrayToObject(this.queries),
                    qindex,
                    mindex,
                );
                let subNodes = tsdbQuery[0].executionGraph.filter(
                    (d) => d.id.indexOf(dsId) === 0,
                );
                dsId = subNodes[subNodes.length - 1].id;
                data.threshold.singleMetric.metricId = dsId;
                data.threshold.isNagEnabled =
                    data.threshold.nagInterval !== '0' ? true : false;

                if (this.suppressConfig.metricId) {
                    const [qqindex, mmindex] = this.utils.getMetricIndexFromId(
                        this.suppressConfig.metricId,
                        this.queries,
                    );
                    dsId = this.utils.getDSId(
                        this.utils.arrayToObject(this.queries),
                        qqindex,
                        mmindex,
                    );
                    subNodes = tsdbQuery[0].executionGraph.filter(
                        (d) => d.id.indexOf(dsId) === 0,
                    );
                    dsId = subNodes[subNodes.length - 1].id;
                    ...
                    data.threshold.suppress.reportingInterval =
                        data.threshold.suppress.timeSampler !==
                        'all_of_the_times'
                            ? null
                            : this.suppressConfig.reportingInterval;
                }
                // eslint-disable-next-line max-len
                data.threshold.autoRecoveryInterval =
                    data.threshold.autoRecoveryInterval !== 'null'
                        ? data.threshold.autoRecoveryInterval
                        : null;
                // eslint-disable-next-line max-len
                data.threshold.singleMetric.reportingInterval =
                    data.threshold.singleMetric.timeSampler ===
                        'all_of_the_times' &&
                    data.threshold.singleMetric.reportingInterval !== null
                        ? data.threshold.singleMetric.reportingInterval
                        : null;
                // eslint-disable-next-line max-len
                data.threshold.singleMetric.requiresFullWindow = data.threshold
                    .singleMetric.reportingInterval
                    ? true
                    : false;
                if (this.data.threshold.subType === 'periodOverPeriod') {
                    ...
                    data.threshold.delayEvaluation =
                        this.periodOverPeriodConfig.delayEvaluation;
                    data.threshold.periodOverPeriod = {
                        ...this.periodOverPeriodConfig.periodOverPeriod,
                    };
                    data.threshold.periodOverPeriod.metricId = subNodes[0].id; // metric/expression node
                    data.threshold.periodOverPeriod.queryIndex =
                        dataThresholdCopy.singleMetric.queryIndex;
                }
                break;
            case 'healthcheck':
                data.threshold.missingDataInterval =
                    data.threshold.notifyOnMissing === 'true'
                        ? data.threshold.missingDataInterval
                        : null;
        ...
        this.configChange.emit({
            action: 'SaveAlert',
            namespace: this.data.namespace,
            dashboard: this.dashboardToCancelTo,
    ...
    saveSnapshot() {
        ...
        for (let i = 0; i < queries.length; i++) {
            for (let j = 0; j < queries[i].metrics.length; j++) {
                // eslint-disable-next-line max-len
                queries[i].metrics[j].settings.visual.visible =
                    !this.thresholdSingleMetricControls.metricId.value ||
                    queries[i].metrics[j].id ===
                        this.thresholdSingleMetricControls.metricId.value;
            }
        }
        const dConfig: any = {
    ...
    */
    ...
    setEventAlertGroupBy(arr) {
        this.alertForm
            .get('queries')
            .get('eventdb')
            ['controls'][0].get('groupBy')
            .setValue(arr, { emitEvent: false });
    ...
    alertRecipientsUpdate(event: any) {
        ...
        if (this.notificationRecipients.value.opsgenie && !event.opsgenie) {
            this.alertForm['controls'].notification
                .get('opsgeniePriority')
                .setValue('');
            this.alertForm['controls'].notification
                .get('opsgenieAutoClose')
                .setValue(false);
            this.alertForm.get('notification')['controls']['opsgenieTags'] =
                this.fb.array([]);
        }

        if (this.notificationRecipients.value.pagerduty && !event.pagerduty) {
            this.alertForm['controls'].notification
                .get('pagerdutyAutoClose')
                .setValue(false);
        }
    ...
    simulateMetricDataSource(queryMetrics: any[]) {
        ...
        const metrics = [];
        queryMetrics
            .filter((d) => d.expression === undefined)
            .forEach((metric, i) => {
                metrics.push({
                    indexLabel: 'm' + (i + 1),
                    type: 'metric',
    ...
    getHashMetricIdUserAliases(query: any) {
        ...
        for (let i = 0; i < query.metrics.length; i++) {
            const alias =
                query.metrics[i].expression === undefined
                    ? 'm' + ++metricIndex
                    : 'e' + ++expressionIndex;
    ...
    setAlertEvaluationLink() {
        const url = this.appConfig.getConfig().alert_history_url + this.data.id;
    ...
    private openAlertNameDialog() {
        ...
        this.nameAlertDialog = this.dialog.open(
