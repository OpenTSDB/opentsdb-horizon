...
@Component({
    ...
    templateUrl:
        './alert-details-metric-period-over-period-preview.component.html',
    styleUrls: [
        './alert-details-metric-period-over-period-preview.component.scss',
    ],
    encapsulation: ViewEncapsulation.None,
})
export class AlertDetailsMetricPeriodOverPeriodPreviewComponent
implements OnInit, OnChanges, OnDestroy {
    ...
    set thresholdConfig(value: any) {
        ...
        if (
            ...
            this.thresholdConfig.periodOverPeriod &&
            this.thresholdData
        ) {
            this.thresholdData = this.getThresholdData(
                this.chartData.ts,
                this.observedIndex,
            );
        }
    ...
    ngOnChanges(changes: SimpleChanges) {
        if (
            changes.nQueryDataLoading &&
            changes.nQueryDataLoading.currentValue
        ) {
            // loading data
            this.observedIndex = -1;
            ...
            changes.chartData &&
            changes.nQueryDataLoading &&
            changes.nQueryDataLoading.currentValue === 0
        ) {
            // new data
    ...
    extractZoomedTimeseries(zConfig, data) {
        ...
        zConfig.end =
            Math.ceil(zConfig.end) >= endTime ? -1 : Math.floor(zConfig.end);

        // extract out in-range timeseries
        const zoomedTS = [];
        for (const ts of data.ts) {
            if (
                ts[0].getTime() / 1000 > zConfig.start &&
                ts[0].getTime() / 1000 < zConfig.end
            ) {
                zoomedTS.push(ts);
            }
        }
    ...
    handleZoomQuery(zConfig) {
        ...
        if (zConfig.isZoomed && n > 0) {
            ...
            // get zoome-in data
            this.chartData = this.extractZoomedTimeseries(
                zConfig,
                this.chartData,
            );
        } else if (!this.isChartDataZoomedOutEmpty()) {
            // zoomed out
    ...
    handleZoomThreshold(zConfig) {
        ...
        if (zConfig.isZoomed && n > 0) {
            ...
            // get zoome-in data
            this.thresholdData = this.extractZoomedTimeseries(
                zConfig,
                this.thresholdData,
            );
        } else if (!this.isThresholdDataZoomedOutEmpty()) {
            // zoomed out
    ...
    reloadPreview() {
        ...
        if (
            ...
            this.chartData.ts &&
            this.chartData.ts[0] &&
            this.chartData.ts[0].length === 3 &&
            this.nQueryDataLoading === 0
        ) {
            this.timeSeriesClicked({ timeSeries: '1' });
        } else if (this.observedIndex !== -1) {
            this.timeSeriesClicked({
                timeSeries: this.observedIndex.toString(),
            });
        }
    }
    ...
    timeSeriesClicked(e) {
        ...
        if (this.observedIndex !== -1) {
            ...
            this.thresholdOptions.visibility = [
                'true',
                'true',
                'true',
                'true',
            ...
            this.thresholdOptions.series = this.getThresholdSeriesOptions(
                this.options.series[
                    this.observedIndexToChartIndex[this.observedIndex]
                ],
            );
    ...
    getThresholdSeriesOptions(selectedTimeSeriesOption) {
        ...
        series[2] = this.setLabelAndColor(
            baseOption,
            'Expected Value',
            '#000000',
        );
        series[3] = this.setLabelAndColor(
            baseOption,
            'Upper Bad Threshold',
            '#ff0000',
            'dashed',
        );
        series[4] = this.setLabelAndColor(
            baseOption,
            'Upper Warning Threshold',
            '#ffa500',
            'dashed',
        );
        series[5] = this.setLabelAndColor(
            baseOption,
            'Lower Warning Threshold',
            '#ffa500',
            'dashed',
        );
        series[6] = this.setLabelAndColor(
            baseOption,
            'Lower Bad Threshold',
            '#ff0000',
            'dashed',
    ...
    getObservedOptions() {
        ...
        for (const serie of _series) {
            if (
                !originalSeries[serie].metric.endsWith('prediction') &&
                originalSeries[serie].tags['_anomalyModel'] !== 'OlympicScoring'
            ) {
    ...
    getThresholdData(allTimeSeries, timeSeriesIndex: number) {
        ...
        if (this.thresholdConfig && this.thresholdConfig.periodOverPeriod) {
            const selectedTimeseries: any[] = this.getTimeSeriesFromIndex(
                allTimeSeries,
                this.observedIndexToChartIndex[timeSeriesIndex],
            );
            const expectedTimeSeries: any[] = this.getExpectedTimeSeries(
                allTimeSeries,
                this.observedIndexToChartIndex[timeSeriesIndex],
                this.options,
            );
            const upperBadTimeSeries: any[] = this.getThresholdTimeSeries(
                expectedTimeSeries,
                parseFloat(
                    this.thresholdConfig.periodOverPeriod.badUpperThreshold,
                ),
                this.thresholdConfig.periodOverPeriod.upperThresholdType,
                'above',
            );
            const upperWarningTimeSeries: any[] = this.getThresholdTimeSeries(
                expectedTimeSeries,
                parseFloat(
                    this.thresholdConfig.periodOverPeriod.warnUpperThreshold,
                ),
                this.thresholdConfig.periodOverPeriod.upperThresholdType,
                'above',
            );
            const lowerWarningTimeSeries: any[] = this.getThresholdTimeSeries(
                expectedTimeSeries,
                parseFloat(
                    this.thresholdConfig.periodOverPeriod.warnLowerThreshold,
                ),
                this.thresholdConfig.periodOverPeriod.lowerThresholdType,
                'below',
            );
            const lowerBadTimeSeries: any[] = this.getThresholdTimeSeries(
                expectedTimeSeries,
                parseFloat(
                    this.thresholdConfig.periodOverPeriod.badLowerThreshold,
                ),
                this.thresholdConfig.periodOverPeriod.lowerThresholdType,
                'below',
            );
    ...
    getExpectedTimeSeries(allTimeSeries, timeseriesIndex: number, options) {
        ...
        for (const serie of _series) {
            if (
                deepEqual(options.series[serie].tags, metricTags) &&
                parseInt(serie, 10) !== timeseriesIndex
            ) {
    ...
    getThresholdTimeSeries(
        ...
        for (const dataPoint of timeseries) {
            if (thresholdType === 'value' && thresholdDirection === 'above') {
                ...
                thresholdTimeSeries.push(
                    dataPoint * (1 + thresholdValue / 100),
                );
            } else if (
                thresholdType === 'percent' &&
                thresholdDirection === 'below'
    ...
    setSeriesVisibilityConfig(index: number, visibility: boolean) {
        ...
        this.observedOptions.visibilityHash[options.series[index + 1].hash] =
