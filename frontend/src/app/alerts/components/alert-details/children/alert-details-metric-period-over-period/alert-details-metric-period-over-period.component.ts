...
export class AlertDetailsMetricPeriodOverPeriodComponent implements OnInit {
    ...
    setDefaultConfig() {
        ...
        this.config.periodOverPeriod.queryType =
            this.config.periodOverPeriod.queryType || 'tsdb';
        this.config.periodOverPeriod.slidingWindow =
            this.config.periodOverPeriod.slidingWindow || '300';
        this.config.periodOverPeriod.period =
            this.config.periodOverPeriod.period || '3600';
        this.config.periodOverPeriod.lookbacks =
            this.config.periodOverPeriod.lookbacks || '6';
        ...
        this.config.periodOverPeriod.warnLowerThreshold =
            this.config.periodOverPeriod.warnLowerThreshold || '';
        this.config.periodOverPeriod.upperThresholdType =
            this.config.periodOverPeriod.upperThresholdType || 'value';
        this.config.periodOverPeriod.lowerThresholdType =
            this.config.periodOverPeriod.lowerThresholdType || 'value';
        this.config.periodOverPeriod.highestOutliersToRemove =
            this.config.periodOverPeriod.highestOutliersToRemove || '1';
    ...
    updateConfig(prop, val) {
        ...
        if (
            prop === 'badUpperThreshold' ||
            prop === 'warnUpperThreshold' ||
            prop === 'badLowerThreshold' ||
            prop === 'warnLowerThreshold'
        ) {
        ...
        if (
            prop === 'lookbacks' ||
            prop === 'highestOutliersToRemove' ||
            prop === 'lowestOutliersToRemove' ||
            prop === 'slidingWindow' ||
            prop === 'period'
        ) {
            requeryData = true;
    ...
    thresholdValidator(thresholdToCompare: string): ValidatorFn {
        return (control: AbstractControl): { [key: string]: any } | null => {
            ...
            if (control.value === '' || control.value === null) {
                forbidden = false;
            } else if (thresholdToCompare.includes('bad')) {
                // compare warn value to bad threshold
                forbidden = this.isValueLargerThanThreshold(
                    thresholdToCompare,
                    control.value,
                    Number.MAX_SAFE_INTEGER,
                );
            } else {
                // compare bad value to warn threshold
                forbidden = !this.isValueLargerThanThreshold(
                    thresholdToCompare,
                    control.value,
                    -1,
    ...
    outliersValidator(): ValidatorFn {
        return (control: AbstractControl): { [key: string]: any } | null => {
            let forbidden = false;
            const totalOutliersToRemove =
                Number(this.config.periodOverPeriod['lowestOutliersToRemove']) +
                Number(this.config.periodOverPeriod['highestOutliersToRemove']);
            if (
                totalOutliersToRemove >=
                Number(this.config.periodOverPeriod['lookbacks'])
            ) {
                forbidden = true;
            }
            return forbidden
    ...
    isValueLargerThanThreshold(
        threshold: string,
        value: string,
        defaultValue: number,
    ): boolean {
        const thresholdValue: number =
            this.config.periodOverPeriod[threshold] === ''
                ? defaultValue
                : Number(this.config.periodOverPeriod[threshold]);
