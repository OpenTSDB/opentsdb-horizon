...
export class AlertDetailsSuppressConfigComponent implements OnInit, OnChanges {
    @HostBinding('class.alert-details-suppress-config-component')
    ...
    ngOnChanges(changes: SimpleChanges) {
        if (
            ...
            (changes.queries &&
                changes.queries.previousValue &&
                changes.queries.currentValue)
        ) {
            this.validateForm();
        }
    }

    setupForm(update = false): void {
        ...
        this.suppressForm.valueChanges.subscribe((formval) => {
            this.validateForm();
    ...
    validateForm(): void {
        ...
        if (formval.metricId) {
            ...
            if (this.tags.length && !suppressTags.length) {
                this.suppressForm
                    .get('metricId')
                    .setErrors({ tagRequired: true });
            }
            if (
                this.tags.length &&
                suppressTags.length &&
                !this.utils.isArraySubset(this.tags, suppressTags)
            ) {
                this.suppressForm
                    .get('metricId')
                    .setErrors({ tagSubset: true });
            }
            if (
                formval.comparisonOperator !== 'missing' &&
                formval.threshold === null
            ) {
                this.suppressForm
                    .get('threshold')
            ...
            if (
                formval.timeSampler === 'all_of_the_times' &&
                formval.reportingInterval <= 0
            ) {
                this.suppressForm
    ...
    updateSuppressConfig(field, v): void {
        if (field === 'name' && !v) {
            this.suppressForm.get('threshold').setErrors(null);
            this.suppressForm.get('reportingInterval').setErrors(null);
        }
