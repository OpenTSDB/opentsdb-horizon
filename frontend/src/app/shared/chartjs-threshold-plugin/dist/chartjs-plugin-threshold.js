...
(function () {
...
})()(
    {
        ...
        2: [
            function (require, module, exports) {
                ...
                ThresholdLine.prototype.update = function () {
                    ...
                    this.y1 =
                        chart.scales[this.scaleId].getPixelForValue(
                            this.value
                        ) + d;
                    this.x2 =
                        this.x1 +
        ...
        4: [
            function (require, module, exports) {
                ...
                module.exports = function (Chart) {
                    ...
                    function initialize(chart) {
                        ...
                        chart.options.threshold = chartHelpers.configMerge(
                            Chart.Threshold.defaults,
                            chart.options.threshold || {}
                        );
                        chart.threshold = {
                            elements: {},
                    ...
                    function setSize(chart) {
                        ...
                        chart.threshold.overlayCanvas.width =
                            chart.width * pixelRatio;
                        chart.threshold.overlayCanvas.height =
                            chart.height * pixelRatio;
                        chart.threshold.overlayCanvas
                            .getContext("2d")
                            .scale(pixelRatio, pixelRatio);
                        chart.threshold.overlayCanvas.style.height =
                            chart.height + "px";
                        chart.threshold.overlayCanvas.style.width =
                            chart.width + "px";
                    }
                    ...
                    function updateConfig(chart) {
                        ...
                        lineConfig =
                            chart.options.threshold &&
                            chart.options.threshold.line
                                ? chartHelpers.configMerge(
                                      Chart.Threshold.lineDefaults,
                                      chart.options.threshold.line
                                  )
                                : Chart.Threshold.lineDefaults;
                        //ctx = chart.options.threshold.draw ? chart.threshold.overlayCanvas.getContext('2d') : chart.ctx;
                        chart.threshold.overlayCanvas.style.display = chart
                            .options.threshold.draw
                            ? "block"
                            : "none";
                     ...
                     */
                    function draw(chart) {
                        var ctx = chart.options.threshold.draw
                            ? chart.threshold.overlayCanvas.getContext("2d")
                            : chart.ctx;
                    ...
                    function select(chart, el) {
                        ...
                        dragging = true;
                        chartHelpers.addEvent(
                            document,
                            "keydown",
                            chart.threshold._winKeyDownEventHandler
                        );
                    }
                    ...
                    function fireEvent(chart) {
                        var evt = new CustomEvent("onThresholdSet", {
                            detail: chart.threshold.elements
                        });
                    ...
                    return {
                        ...
                        afterInit: function (chart) {
                            ...
                            chart.threshold._winKeyDownEventHandler = function (
                                e
                            ) {
                                if (
                                    chart.threshold.selection &&
                                    e.keyCode == 8
                                ) {
                                    delete chart.threshold.elements[
                                        chart.threshold.selection.id
                                    ];
                                    clearSelection(chart);
                                    draw(chart);
                                }
				...
				selects/unselects the line if the line is there in the area */
                            chart.threshold._mouseDownEventHandler = function (
                                e
                            ) {
                                var mx = e.offsetX;
                                var my = e.offsetY;
                                for (var i in chart.threshold.elements) {
                                    if (
                                        chart.threshold.elements[i].inRange(
                                            mx,
                                            my,
                                            selectionBorder
                                        )
                                ...
                                if (chart.options.threshold.draw) {
                                    if (chart.threshold.selection) {
                                        ...
                                        Object.keys(chart.threshold.elements)
                                            .length <
                                        chart.options.threshold.maxLines
                                    ) {
                                        var id = "line-" + new Date().getTime();
                                        var options = {
                                            id: id,
                                            scaleId:
                                                chart.options.threshold.scaleId,
                                            value: chart.scales[
                                                chart.options.threshold.scaleId
                                            ].getValueForPixel(e.offsetY),
                                        };
                                        chart.threshold.elements[id] =
                                            new ThresholdLine(
                                                chart,
                                                chartHelpers.configMerge(
                                                    Chart.Threshold
			...
			*/
                            chart.threshold._mouseMoveEventHandler = function (
                                e
                            ) {
                                if (dragging) {
                                    if (
                                        e.offsetY <= chart.chartArea.bottom &&
                                        e.offsetY >= chart.chartArea.top
                                    ) {
                                        chart.threshold.selection.value =
                                            chart.scales[
                                                chart.threshold.selection
                                                    .scaleId
                                            ].getValueForPixel(e.offsetY);
                            ...
                            chart.threshold._mouseUpEventHandler = function (
                                e
                            ) {
                                if (dragging) {
                                    dragging = false;
                            ...
                            chartHelpers.addEvent(
                                chart.threshold.overlayCanvas,
                                "mousedown",
                                chart.threshold._mouseDownEventHandler
                            );
                            chartHelpers.addEvent(
                                chart.threshold.overlayCanvas,
                                "mousemove",
                                chart.threshold._mouseMoveEventHandler
                            );
                            chartHelpers.addEvent(
                                chart.threshold.overlayCanvas,
                                "mouseup",
                                chart.threshold._mouseUpEventHandler
                            );
                        },
                        ...
                        afterUpdate: function (chart) {
                            ...
                            if (
                                chart.options.threshold &&
                                chart.options.threshold.thresholds &&
                                !chart.threshold.firstRun
                            ) {
                                var thresholds =
                                    chart.options.threshold.thresholds;
                                var width =
