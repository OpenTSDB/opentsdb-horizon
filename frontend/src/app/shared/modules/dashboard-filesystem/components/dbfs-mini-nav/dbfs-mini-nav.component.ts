...
export class DbfsMiniNavComponent implements OnInit, OnDestroy {
    ...
    get selectEnabled(): boolean {
        if (this.mode === 'select') {
            if (!this.panels[this.panelIndex].selectEnabled) {
                if (
                    this.selected &&
                    this.folders[this.selected].selectEnabled
                ) {
                    return true;
                }
            }
    ...
    private miniNavInit() {
        ...
        if (this.type === 'file') {
            this.originDetails = this.store.selectSnapshot(
                DbfsResourcesState.getFile(initialPath),
            );
        } else {
            this.originDetails = this.store.selectSnapshot(
                DbfsResourcesState.getFolder(initialPath),
            );
        }

        // setup root panel
        const miniRoot = this.store.selectSnapshot(
            DbfsResourcesState.getFolder(':mini-root:'),
        );
        const miniRootPanel = this.dbfsUtils.normalizePanelFolder(
            miniRoot,
            false,
            false,
        );
        ...
        miniRootPanel.subfolders.forEach((item) => {
            ...
            const folder = this.store.selectSnapshot(
                DbfsResourcesState.getFolder(item),
            );
            const folderPanel = this.dbfsUtils.normalizePanelFolder(
                folder,
                enabled,
                enabled,
            );
        ...
        const userFolder = this.store.selectSnapshot(
            DbfsResourcesState.getFolder(miniRoot.subfolders[0]),
        );
        ...
        const mbrNamespaces = this.store.selectSnapshot(
            DbfsResourcesState.getFolder(':member-namespaces:'),
        );
        const mbrNamespacesPanel = this.dbfsUtils.normalizePanelFolder(
            mbrNamespaces,
            false,
            false,
        );
        ...
        if (pathDetails.type === 'namespace') {
            ...
            const userNoDisplay =
                this.mode === 'save'
                    ? false
                    : userFolder.fullPath === this.originDetails.fullPath;
            const userFolderPanel = this.dbfsUtils.normalizePanelFolder(
                userFolder,
                true,
                true,
                userNoDisplay,
            ...
            for (let i = 0; i < userFolderPanel.subfolders.length; i++) {
                ...
                if (item) {
                    ...
                    const folderPanel = this.dbfsUtils.normalizePanelFolder(
                        ...
                        true,
                        true,
                        folder.fullPath === this.originDetails.fullPath,
                    );
                    this.folders[folderPanel.fullPath] = folderPanel;
                }
            }
        ...
        const topFolder = this.store.selectSnapshot(
            DbfsResourcesState.getFolder(pathPrefix),
        );
        const topFolderNoDisplay =
            this.mode === 'save'
                ? false
                : topFolder.fullPath === this.originDetails.fullPath;
        const topFolderPanel = this.dbfsUtils.normalizePanelFolder(
            topFolder,
            true,
            true,
            topFolderNoDisplay,
        ...
        if (pathParts.length > 0) {
            for (let i = 0; i < pathParts.length; i++) {
                ...
                for (let j = 0; j < folder.subfolders.length; j++) {
                    ...
                    if (item) {
                        const subfolder = this.store.selectSnapshot(
                            DbfsResourcesState.getFolder(item),
                        );
                        const subfolderNoDisplay =
                            this.mode === 'save'
                                ? false
                                : subfolder.fullPath ===
                                  this.originDetails.fullPath;
                        const subfolderPanel =
                            this.dbfsUtils.normalizePanelFolder(
                                subfolder,
                                true,
                                true,
                                subfolderNoDisplay,
                        ...
                        if (
                            this.mode === 'save' &&
                            (subfolder.trashFolder ||
                                (subfolder.ownerType === 'user' &&
                                    subfolder.name === '_clipboard_'))
                        ) {
                ...
                this.directorySelected.emit({
                    ...
                    payload: {
                        ...
                        destinationId: this.selected
                            ? this.folders[this.selected].id
                            : this.panels[this.panelIndex].id,
    ...
    folderAction(action: any, folder: any, event: any) {
        switch (action) {
            ...
            case 'selectFolder':
                ...
                if (
                    (this.mode === 'move' &&
                        (pathParts.length === 2 || !folder.moveEnabled)) ||
                    ((this.mode === 'save' || this.mode === 'select') &&
                        (pathParts.length === 2 || !folder.selectEnabled))
                ) {
    ...
    private fetchFolder(folder: any): Promise<any> {
        ...
        if (topFolder && topFolder.type && topFolder.value) {
            ...
            apiUrl =
                this.appConfig.getConfig().configdb +
                '/dashboard' +
                folder.path;
        }
        ...
        this.fetchFolder(folder)
            .then((resource) => {
                const folderPanel = this.dbfsUtils.normalizePanelFolder(
                    ...
                    true,
                    true,
                    resource.fullPath === this.originDetails.fullPath,
                );
                folderPanel.loaded = true;

                // just to be sure the name carries over
                if (
                    folderPanel.ownerType === 'namespace' &&
                    folderPanel.topFolder === true
                ) {
                    const nsData = this.store.selectSnapshot(
                        DbfsResourcesState.getNamespacesData,
                    );
                    folderPanel.name = nsData[folderPanel.namespace].name;
                }

                if (
                    folderPanel.ownerType === 'user' &&
                    folderPanel.topFolder === true
                ) {
                ...
                folderPanel.subfolders.sort((a: any, b: any) => this.utils.sortAlphaNum(a.name, b.name));

                for (const subfolder of folderPanel.subfolders) {
                    subfolders.push(subfolder.fullPath);
                    const subfolderPanel = this.dbfsUtils.normalizePanelFolder(
                        subfolder,
                        true,
                        true,
                        subfolder.fullPath === this.originDetails.fullPath,
                    );
                    this.folders[subfolderPanel.fullPath] = subfolderPanel;

                    if (
                        this.mode === 'save' &&
                        (subfolderPanel.trashFolder ||
                            (subfolderPanel.ownerType === 'user' &&
                                subfolderPanel.name === '_clipboard_'))
                    ) {
                        delete this.folders[subfolderPanel.fullPath];
                    }
                }
