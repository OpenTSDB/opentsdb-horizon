...
export class DbfsComponent implements OnInit, OnDestroy {
    ...
    @HostBinding('class.panel-content') private _panelContentClass = true;

    @ViewChildren('moreMenuTrigger', { read: MatMenuTrigger })
    moreTriggers: QueryList<MatMenuTrigger>;
    @ViewChildren('miniNavTrigger', { read: MatMenuTrigger })
    miniNavTriggers: QueryList<MatMenuTrigger>;
    ...
    @Select(DbfsResourcesState.getResourceAction)
    resourceAction$: Observable<any>;
    ...
    // VIEW CHILDREN
    @ViewChild(NavigatorPanelComponent, { static: true })
    ...
    ngOnInit() {
        ...
        this.subscription.add(
            this.loadedDashboardId$.subscribe((id) => {
        ...
        this.subscription.add(
            this.currentPanelIndex$.subscribe((idx) => {
                this.currentPanelIndex = idx;
            }),
        );

        this.subscription.add(
            this.panelTab$.subscribe((tab) => {
                this.currentPanelTab = tab;
            }),
        );

        this.subscription.add(
            this.dynamicLoaded$.subscribe((loaded) => {
        ...
        this.subscription.add(
            this.namespacesData$.subscribe((namespaces) => {
                ...
                this.namespacesDataSource = new MatTableDataSource(
                    this.namespacesList,
                );
                this.namespacesDataSource.filterPredicate = (
                    data: any,
                    filter: string,
        ...
        this.subscription.add(
            this.userFavorites$.subscribe((favorites) => {
                ...
                this.userFavoritesDataSource = new MatTableDataSource(
                    this.userFavorites,
                );
                this.userFavoritesDataSource.filterPredicate = (
                    data: any,
                    filter: string,
                ) => data.name.toLowerCase().includes(filter);
            }),
        ...
        this.subscription.add(
            this.userRecents$.subscribe((recents) => {
                this.userRecents = recents ? recents : [];
                this.userRecentsDataSource = new MatTableDataSource(
                    this.userRecents,
                );
                this.userRecentsDataSource.filterPredicate = (
                    data: any,
                    filter: string,
                ) => data.name.toLowerCase().includes(filter);
            }),
        ...
        this.subscription.add(
            this.panelAction$.subscribe((action) => {
                switch (action.method) {
                    case 'goNextPanel':
                        setTimeout(
                            function () {
                    ...
                    case 'changePanelTab':
                        ...
                        switch (action.tab) {
                            case 'favorites':
                                setTimeout(
                                    function () {
                                        this.navPanel.resetTo(
                                            0,
                                            this.loadUserFavoritesPanel.bind(
                                                this,
                                            ),
                                        );
                                    }.bind(self),
                            ...
                            case 'recent':
                                setTimeout(
                                    function () {
                                        this.navPanel.resetTo(
                                            0,
                                            this.loadUserRecentPanel.bind(this),
                                        );
                                    }.bind(self),
                                    100,
                                );
                                ...
                                if (this.namespacesListLoaded) {
                                    ...
                                    setTimeout(
                                        function () {
                                            this.navPanel.resetTo(
                                                0,
                                                this.loadAllNamespacesPanel.bind(
                                                    this,
                                                ),
                                            );
                                        }.bind(self),
        ...
        this.subscription.add(
            this.curDashboardId$.subscribe((db) => {
    ...
    getResource(path: string, type?: string) {
        const resource = this.store.selectSnapshot<any>(
            DbfsResourcesState.getFolderResource(path),
        );
        return resource;
    }
    ...
    findMoreMenuTrigger(id: any, type: any): MatMenuTrigger {
        const trigger = this.moreTriggers.find((item) => item.menuData.type === type && item.menuData.data.id === id);
        return trigger || null;
    }

    findMiniNavTrigger(id: any, type: any): MatMenuTrigger {
        const trigger = this.miniNavTriggers.find((item) => item.menuData.type === type && item.menuData.data.id === id);
        return trigger || null;
    }
    ...
    miniNavCancel(event: any) {
        const trigger: MatMenuTrigger = this.findMiniNavTrigger(
            event.id,
            event.type,
        );
        trigger.closeMenu();
    ...
    miniNavSelected(event: any) {
        switch (event.action) {
            case 'miniNavMove':
                this.store.dispatch(
                    new DbfsMoveResource(
                        event.payload.sourceId,
                        event.payload.destinationId,
                        event.payload.originPath,
                        {},
                    ),
                );
                break;
    ...
    clickMoreMenu(id: number, type: string, event: any) {
        ...
        const mTrigger: MatMenuTrigger = <MatMenuTrigger>(
            this.findMoreMenuTrigger(id, type)
        );
    ...
    clickMoveMenu(id: number, type: string, event: any) {
        event.stopPropagation();
        const mTrigger: MatMenuTrigger = <MatMenuTrigger>(
            this.findMiniNavTrigger(id, type)
        );
        if (mTrigger) {
            mTrigger.toggleMenu();
            // close the more menu
    ...
    loadAllNamespacesPanel(callback?: any) {
        if (!this.namespacesListLoaded) {
            this.store
                .dispatch(new DbfsLoadNamespacesList({}))
                .subscribe(() => {
                    if (callback) {
                        callback();
                    }
    ...
    loadAllUsersPanel(callback?: any) {
        if (!this.usersListLoaded) {
            this.store.dispatch(new DbfsLoadUsersList({})).subscribe(() => {
                if (callback) {
                    callback();
                }
            });
    ...
    loadUserFavoritesPanel() {
        if (!this.userFavoritesListLoaded) {
            this.store.dispatch(new DbfsLoadUserFavorites(null, {}));
        }
    }

    loadUserRecentPanel() {
        if (!this.userRecentListLoaded) {
            this.store.dispatch(new DbfsLoadUserRecents(null, null, {}));
        }
    }
    ...
    removeFolders() {
        this.store.dispatch(
            new DbfsDeleteFolder(this.foldersToRemove, {
    ...
    folderInputSave(panelIndex: number, folder?: any) {
        ...
        const panelFolder =
            this.folders[this.panels[panelIndex].folderResource];
        ...
        if (this.edit.mode === 'create') {
            ...
            this.store.dispatch(new DbfsCreateFolder(payload, resourceAction));
        }
        ...
        if (this.edit.mode === 'edit') {
            ...
            this.resetEdit();
            this.store.dispatch(
                new DbfsUpdateFolder(payload, folder.fullPath, resourceAction),
            );
        }
    }
    ...
    folderMenuAction(action: string, folder: any, event?: any) {
        switch (action) {
            ...
            case 'deleteFolder':
                this.store.dispatch(
                    new DbfsDeleteFolder([folder.fullPath], {
    ...
    fileMenuAction(action: string, file: any, event?: any) {
        switch (action) {
            ...
            case 'deleteDashboard':
                this.store.dispatch(
                    new DbfsDeleteDashboard(file.fullPath, {
    ...
    gotoFolder(path: string) {
        if (!this.bulkEdit) {
            const folder = this.store.selectSnapshot<any>(
                DbfsResourcesState.getFolderResource(path),
            );

            if (
                folder.fullPath === ':user-recent:' ||
                folder.fullPath === ':user-favorites:'
            ) {
                ...
                if (!folder.loaded && !folder.synthetic) {
                    panelAction.method = folder.topFolder
                        ? 'loadTopFolder'
                        : 'loadSubFolder';
    ...
    gotoTopFolder(key: string, type: string) {
        ...
        if (folder.notFound) {
            setTimeout(
                function () {
                    this.store.dispatch(
                        new DbfsAddPlaceholderFolder(path, {
                            method: 'gotoFolder',
                            args: path,
                        }),
                    );
