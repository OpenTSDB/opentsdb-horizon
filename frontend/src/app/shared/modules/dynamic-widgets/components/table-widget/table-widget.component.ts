...
export class TableWidgetComponent implements OnInit, AfterViewInit, OnDestroy {
                ...
                if (message && message.id === this.widget.id) {
                    switch (message.action) {
                        case 'updatedWidgetGroup':
                            ...
                            this.data = this.dataTransformer.openTSDBToTable(
                                this.widget,
                                this.options,
                                rawdata,
                            );
                            ...
                            this.displayedColumns = this.util.deepClone(
                                this.options.displayColumns,
                            );
                            this.displayedColumnsIds = [
                                ...this.displayedColumns.map((d) => d.id),
                            ];
    ...
    sortingDataAccessor(data, colId) {
        switch (colId) {
            ...
            case 'tag':
                return data[colId];
            default:
                return parseFloat(data[colId + ':raw']);
        }
    }
    ...
    unsetMetricSummarizer() {
        if (
            this.widget.settings.visual.layout !== 'metrics:tags' &&
            this.widget.settings.visual.layout !== 'tags:metrics'
        ) {
