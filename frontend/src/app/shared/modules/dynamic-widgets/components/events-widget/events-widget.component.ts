...
export class EventsWidgetComponent implements OnInit, OnDestroy, OnChanges {
    ...
    ngOnInit() {
        this.widget = {
            ...this.util.setDefaultEventsConfig(this.widget, true),
        };
        this.getEvents();
    ...
    getEvents() {
        this.interCom.requestSend({
            id: this.widget.id,
            action: 'getEventData',
            payload: this.util.deepClone({
                eventQueries: this.widget.eventQueries,
                limit:
                    this.mode === 'edit'
                        ? this.previewEventsCount
                        : this.eventsCount,
            }),
        });
    }
