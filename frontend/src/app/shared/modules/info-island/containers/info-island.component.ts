...
@Component({
    ...
    animations: [
        trigger('infoIslandAnimation', [
            ...
            transition(
                '* <=> void',
                animate('400ms cubic-bezier(0.4, 0, 0.1, 1)'),
            ),
        ]),
    ],
})
export class InfoIslandComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    @ViewChild('islandContainer', { read: ElementRef, static: true })
    private _islandContainer: ElementRef;

    @ViewChild('islandContainer', { read: CdkDrag, static: true })
    _dragContainer: CdkDrag;

    @ViewChild('islandToolbar', { read: ViewContainerRef, static: true })
    private _islandToolbar: ViewContainerRef;
    get islandToolbar(): ViewContainerRef {
        return this._islandToolbar;
    }

    // resizers Elements
    @ViewChildren('resizerEl', { read: ElementRef })
    private _resizers: QueryList<ElementRef>;
    // resizers CdkDrag instances
    @ViewChildren('resizerEl', { read: CdkDrag })
    private _dragHandles: QueryList<ElementRef>;
    ...
    private snapshotWindowResizeSetup() {
        ...
        this.snapResizeSub = this.snapResizeObservable$
            ...
            .subscribe((evt) => {
                ...
                if (!this.userDragged) {
                    const dbContent = document.querySelector(
                        this.options.outerWrap,
                    );
                    const dbSize =
                        this.options.widgetContainerRef.getBoundingClientRect();

                    this.options.width = dbSize.width;
                    this.hostEl.nativeElement.style.width =
    ...
    open(portalRef: Portal<any>, options: any) {
        ...
        const editCheck = this.options.widgetContainerRef.closest(
            '.edit-view-container',
        );
        if (editCheck) {
            this.snapshotWindowResizeSetup();
    ...
    dragIslandWindow(event: any) { /* do nothing */ }
    ...
    dragResizeStart(e: any) {
        const element = this._islandContainer.nativeElement;
        this.origDims.width = parseFloat(
            getComputedStyle(element, null)
                .getPropertyValue('width')
                .replace('px', ''),
        );
        this.origDims.height = parseFloat(
            getComputedStyle(element, null)
    ...
    dragResizeMove(e: any) {
        ...
        if (currentResizer.classList.contains('top')) {
            height =
                this.origDims.height -
                (e.pointerPosition.y - this.origDims.pointerPosition.y);
            ...
            width =
                this.origDims.width -
                (e.pointerPosition.x - this.origDims.pointerPosition.x);
        ...
        if (triggerTransform) {
            element.style.transform =
                'translate3d(' + transform.join('px,') + 'px)';
        }
