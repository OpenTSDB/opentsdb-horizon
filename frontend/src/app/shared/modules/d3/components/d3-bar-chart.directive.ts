...
export class D3BarChartDirective implements OnInit, OnChanges {
    ...
    horizontalBarChart() {
        ...
        let yAxisWidth = 0,
            labelHeight = 0;
        chartHeight =
            chartHeight > this.size.height || !this.size.height
                ? chartHeight
                : this.size.height - 7; // -7 avoids the scrollbar
        ...
        for (let i = 0, len = dataset.length; i < len; i++) {
            ...
            dataset[i].formattedValue = this.unitService.convert(
                dataset[i].value,
                unitOptions.unit,
                this.unitService.getNormalizedUnit(
                    dataset[i].value,
                    unitOptions,
                ),
                unitOptions,
            ...
            longText =
                longText.length > dataset[i].formattedValue.length
                    ? longText
                    : dataset[i].formattedValue;
        }
        ...
        const mousemove = function (d) {
            ...
            const nunit = self.unitService.getNormalizedUnit(
                d.value,
                unitOptions,
            );
            const nValue = self.unitService.convert(
                d.value,
                unitOptions.unit,
                nunit,
                unitOptions,
            );
            const sValue = self.unitService.convert(
                d.value,
                unitOptions.unit,
                tooltipUnitOptions,
                tooltipUnitOptions,
            ...
            const tmpColor = d.color === 'auto' ? '#000000' : d.color;
            const tcContrast = self.utils.findContrastColor(tmpColor);
            const colorContrast: string =
                tcContrast.type === 'white' ? 'rgb(255,255,255)' : 'rgb(0,0,0)';
        ...
        // this.host.html('');
        const tooltip = d3
            .select(this.element.nativeElement.parentNode.parentNode)
            .select('.tooltip');
        ...
        setTimeout(() => {
            ...
            svg.append('text')
                ...
                .text(longText)
                .each(function () {
                    yAxisWidth = this.getBBox().width;
                    labelHeight = Math.floor(this.getBBox().height);
                })
                .remove();
            const fontSize =
                barHeight >= labelHeight ? '1em' : barHeight * 0.75 + 'px'; // y.bandwidth()  * 0.4 + "px";
            const chartAreawidth =
                this.size.width - yAxisWidth - margin.left - margin.right;
            const dmin =
                dataset.length > 1
                    ? d3.min(dataset, (d: any) => parseFloat(d.value))
                    : Number.MIN_VALUE;
            const min = d3.min(dataset, (d: any) => parseFloat(d.value));
            const offset = Math.sign(min) <= -1 ? -1 * min : 0;
            const max =
                d3.max(dataset, (d: any) => parseFloat(d.value)) + offset;
            ...
            const x = d3
                ...
                .domain([
                    dmin,
                    d3.max(dataset, (d: any) => parseFloat(d.value)),
                ]);
            const g = svg
                .append('g')
                .attr(
            ...
            const bars = g.selectAll('.bar').data(dataset).enter().append('g');
            ...
            bars.append('foreignObject')
                .attr('class', (d: any) => {
                    const tmpClass = 'label';
                    const tmpColor = d.color === 'auto' ? '#000000' : d.color;
                    const tcContrast = this.utils.findContrastColor(tmpColor);
                    return tmpClass + ' ' + tcContrast.type;
                })
                .attr('y', (d, i) => y(i))
                .attr('x', 5)
                .attr('height', barHeight)
                .attr('width', (d: any) => {
                    const txtWidth = this.utils.calculateTextWidth(
                        d.label,
                        '12px',
                        'Helvetica Neue',
                    );
                ...
                .style('background-color', (d: any) => {
                    const tmpColor = d.color === 'auto' ? '#000000' : d.color;
                    const tcContrast = this.utils.findContrastColor(tmpColor);
                    const opacity: any = tcContrast.type === 'white' ? 0.45 : 0;
    ...
    verticalBarChart() {
        ...
        const chartAreaHeight =
            this.size.height - margin.top - margin.bottom - xAxisHeight;
        ...
        const xKey =
            this.options.axes.x && this.options.axes.x.key
                ? this.options.axes.x.key
                : direction === 'horizontal'
                    ? 'value'
                    : 'label';
        const yKey =
            this.options.axes.y && this.options.axes.y.key
                ? this.options.axes.y.key
                : direction === 'horizontal'
                    ? 'label'
                    : 'value';

        const max = dataset.length
            ? d3.max(dataset, (d: any) => Number(d.value))
            : 0;

        const yAxisFormat = this.options.axes.y.format || {
            unit: 'auto',
            precision: 'auto',
        };
        const xAxisFormat = this.options.axes.x.format || {
            unit: 'auto',
            precision: 'auto',
        };
        // eslint-disable-next-line @typescript-eslint/no-this-alias
        ...
        // rerendering causing issue as we clear the chart container. the svg container is not available to calculate the yaxis label width
        setTimeout(() => {
            svg.style('height', this.size.height + 'px'); // chartHeight
            ...
            if (this.options.axes.x.type === 'category') {
                ...
                const key = this.options.axes.x.key
                    ? this.options.axes.x.key
                    : 'value';
                const min = dataset.length
                    ? d3.min(dataset, (d) => Number(d[key]))
                    : 0;
                // NOTE: renamed to max2 to avoid shadowed variable rule
                const max2 = dataset.length
                    ? d3.max(dataset, (d) => Number(d[key]))
                    : 0;
                const m = Math.abs(max2 - min) / dataset.length;
                x = d3
                    .scaleLinear()
                    .domain([min, max + m])
                    .range([0, chartAreawidth]);
            ...
            const yAxis = d3[d3YAxis](y)
                // eslint-disable-next-line max-len
                .tickFormat((d, i) =>
                    self.unitService.convert(
                        d,
                        yAxisFormat.unit,
                        self.unitService.getNormalizedUnit(d, yAxisFormat),
                        yAxisFormat,
                    ),
                );
            ...
            const xAxis = d3[d3XAxis](x)
                ...
                .tickFormat((d, i) =>
                    self.unitService.convert(
                        d,
                        xAxisFormat.unit,
                        self.unitService.getNormalizedUnit(d, xAxisFormat),
                        xAxisFormat,
                    ),
                );
