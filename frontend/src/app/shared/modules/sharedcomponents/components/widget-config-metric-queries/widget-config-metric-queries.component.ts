...
export class WidgetConfigMetricQueriesComponent
implements OnInit, OnDestroy, OnChanges {
    ...
    @ViewChild('queriesContainer', { static: true })
    private queriesContainer: ElementRef;
    ...
    newQueryId = '';
    editQueryId = '';
    selectAllToggle = 'none'; // none/all/some
    tplVariables: any = {};
    ...
    getNewQueryConfig() {
        ...
        const infectiousNan =
            ...
            this.widget.queries[0].settings.infectiousNan
                ? true
                : false;
        ...
        query.id = this.util.generateId(
            3,
            this.util.getIDs(this.widget.queries),
        );
    ...
    handleQueryRequest(message: any) {
        switch (message.action) {
            case 'UpdateQueryVisual':
                ...
                if (
                    message.payload.visual.type &&
                    ['area', 'bar'].includes(message.payload.visual.type)
                ) {
                    for (let i = 0; i < this.widget.queries.length; i++) {
                        for (
                            ...
                            if (['area', 'bar'].includes(vconfig.type)) {
                                message.payload.visual.axis =
                                    vconfig.axis || 'y1';
                                message.payload.visual.stacked =
                                    vconfig.stacked || 'true';
                                break;
                            }
                        }
                    }
            ...
            case 'ToggleQueryVisibility':
                this.widgetChange.emit({
                    id: message.id,
                    action: 'ToggleQueryVisibility',
                });
                break;
            case 'ToggleQueryMetricVisibility':
                this.widgetChange.emit({
                    id: message.id,
                    action: 'ToggleQueryMetricVisibility',
                    payload: { mid: message.payload.mid },
                });
                break;
            case 'CloneQuery':
                this.widgetChange.emit({
            ...
            case 'DeleteQueryMetric':
                ...
                if (
                    expCount === 1 &&
                    this.widget.queries[qindex].metrics[mindex].expression
                ) {
                    this.toggleInfectiousNan(false);
                }
                this.widgetChange.emit({
                    id: message.id,
                    action: 'DeleteQueryMetric',
                    payload: { mid: message.payload.mid },
                });
                break;
            case 'QueryChange':
            ...
            case 'SummarizerChange':
                this.widgetChange.emit({
                    id: message.id,
                    action: 'SummarizerChange',
                    payload: { summarizer: message.payload.summarizer },
                });
                break;
        }
    }
    ...
    updateLabelsForTimeShift(metrics: any[]) {
        for (const metric of metrics) {
            const totalTimeShift = this.util.getTotalTimeShift(
                metric.functions,
            );
            if (totalTimeShift) {
                if (
                    metric.settings.visual.label === '' ||
                    metric.settings.visual.label.startsWith(metric.name + '-')
                ) {
                    metric.settings.visual.label =
                        metric.name + '-' + totalTimeShift;
                }
            } else {
                // timeshift potentially removed
                if (
                    metric.settings.visual.label &&
                    metric.settings.visual.label.startsWith(metric.name + '-')
                ) {
    ...
    reorderQuery(event: any) {
        ...
        this.widgetChange.emit({
            action: 'UpdateQueryOrder',
            payload: { queries: this.widget.queries },
        });
    }
