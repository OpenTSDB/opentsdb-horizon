...
export class WidgetConfigTimeComponent
implements OnInit, OnDestroy, AfterViewInit {
    ...
    ngOnInit() {
        this.minInterval =
            this.widget.settings.time.downsample.minInterval || '';
        this.reportingInterval =
            this.widget.settings.time.downsample.reportingInterval || '';
        this.selectedAggregators =
            this.widget.settings.time.downsample.aggregators ||
            this.selectedAggregators;
        this.startTime = this.widget.settings.time.overrideTime
            ? this.widget.settings.time.overrideTime.start
            : '';
        this.endTime = this.widget.settings.time.overrideTime
            ? this.widget.settings.time.overrideTime.end
            : '';
    ...
    createForm() {
        ...
        const isCustomDownsample =
            this.widget.settings.time.downsample.value === 'custom'
                ? true
                : false;
        const customUnit =
            this.widget.settings.time.downsample.customUnit ||
            this.customDownsampleUnit;
        let minInterval = (
            this.widget.settings.time.minInterval
                ? this.widget.settings.time.minInterval
                : this.minInterval
        ).trim();
        /* eslint-disable max-len */
        let reportingInterval = (
            this.widget.settings.time.reportingInterval
                ? this.widget.settings.time.reportingInterval
                : this.reportingInterval
        ).trim();
        ...
        this.widgetConfigTime = this.fb.group({
            ...
            customDownsampleValue: new UntypedFormControl(
                ...
                [
                    Validators.min(customUnit === 's' ? 10 : 1),
        ...
        this.selectedDownsample_Sub = this.widgetConfigTime
            .get('downsample')
            ...
            .subscribe(
                function (data) {
                    ...
                    if (this.widgetConfigTime.valid) {
                        data.minInterval = data.minInterval
                            ? data.minInterval + data.minIntervalUnit
                            : '';
                        data.reportingInterval = data.reportingInterval
                            ? data.reportingInterval +
                              data.reportingIntervalUnit
                            : '';
                        delete data.reportingIntervalUnit;
                        delete data.minIntervalUnit;
                        if (
                            !this.canOverrideTime ||
                            !data.overrideTime.start ||
                            !data.overrideTime.end
                        ) {
                            delete data.overrideTime;
                        }
                        this.widgetChange.emit({
                            action: 'SetTimeConfiguration',
                            payload: { data: data },
                        });
                    }
                }.bind(this),
