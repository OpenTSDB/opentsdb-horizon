...
export class DygraphsChartDirective implements OnInit, OnChanges, OnDestroy {
    ...
    private lastSeriesHighlighted = -1;
    ...
    ngOnChanges(changes: SimpleChanges) {
        ...
        if (
            ...
            this.options.plugins.length === 1 &&
            this.options.plugins[0] === null
        ) {
            this.options.plugins = [];
        }
        ...
        if (
            !(
                ...
                Object.keys(this.options.labelsDiv).length > 0
            )
        ) {
        ...
        const mouseover = function (e, x, points, row, seriesName) {
            ...
            if (
                e.x > offsetCheck.xMin &&
                e.x < offsetCheck.xMax &&
                e.y > offsetCheck.yMin &&
                e.y < offsetCheck.yMax &&
                (pvSeriesOpts.visible || highlightSeries === seriesName)
            ) {
                // find index in points that matches series
                const pvIdx = points.findIndex(
                ...
                const dunit = self.uConverter.getNormalizedUnit(
                    pv.yval,
                    format,
                );
                ...
                ttData.timestampFormatted = options.labelsUTC
                ...
                if (isNaN(pv.yval)) {
                    ...
                    ttData.valueFormatted = self.uConverter.convert(
                        pv.yval,
                        format.unit,
                        dunit,
                ...
                ttData.metric =
                    tempLabel.indexOf(tags.metric) > -1
                        ? tags.metric
                        : tempLabel;
            ...
            if (
                self.timeseriesLegend.open &&
                self.timeseriesLegend.trackMouse
            ) {
                ...
                this.legendMouseoverTimeout = setTimeout(() => {
                    ...
                    const rawRefIndex = this.rawData_.findIndex(
                        (d: any[]) => d[0] === x,
                    );
                    ...
                    const tickDataOutput = {
                        timestamp: x,
                        when: options.labelsUTC
                            ? moment(x).utc().format('YYYY/MM/DD HH:mm')
                    ...
                    for (let i = 0; i < Object.keys(series).length; i++) {
                        ...
                        const pointIndex = points.findIndex(
                            (p: any) => p.name === seriesIndex.toString(),
                        );
                        ...
                        if (pointIndex !== -1) {
                            ...
                            const rawPointData =
                                this.rawData_[rawRefIndex][seriesIndex];
                        ...
                        if (isNaN(data.data.yval)) {
                            ...
                            data.formattedValue = self.uConverter.convert(
                                data.data.yval,
                                format.unit,
                                dunit,
                                { unit: format.unit, precision: precision },
        ...
        const clickCallback = function (e, x, points) {
            ...
            self.lastTimeseriesHighlighted.emit({
                timeSeries: this.lastSeriesHighlighted,
            });
        };
        ...
        const drawCallback = (dygraph: any) => {
            if (dygraph && dygraph.dateWindow_) {
                this.dateWindow.emit({
                    startTime: dygraph.dateWindow_[0],
                    endTime: dygraph.dateWindow_[1],
                });
                this.startTime = dygraph.dateWindow_[0];
            } else if (dygraph && dygraph.rawData_) {
                // TODO: change - what if we do not have latest data?
                this.dateWindow.emit({
                    startTime: dygraph.rawData_[0][0],
                    endTime: dygraph.rawData_[dygraph.rawData_.length - 1][0],
                });
        ...
        const underlayCallback = (canvas, area, g) => {
            if (this.eventBuckets && this.showEvents) {
                for (const bucket of this.eventBuckets) {
                    let coords;
                    if (this.startTime === bucket.startTime) {
                        // first icon on chart placed at end time of bucket
                        coords = g.toDomCoords(bucket.endTime, 0);
                    } else {
                        // all others placed start + width (so last icon not off the chart)
                        coords = g.toDomCoords(
                            bucket.startTime + bucket.width,
                            0,
                        );
                    }
        ...
        const valueFormatter = function (value, opts) {
            ...
            const precision = format.precision ? format.precision : 2;
            const dunit = self.uConverter.getNormalizedUnit(value, format);
            return self.uConverter.convert(value, format.unit, dunit, {
                unit: format.unit,
                precision: precision,
            });
        };
        ...
        const setHeatmapLegend = function (event, g, x, bucket) {
            ...
            const ttData: any = {
                multigraph:
                    self.multigraph === undefined ? false : self.multigraph,
            };
            ...
            const tooltipData =
                options.series[bucket] && options.series[bucket][x]
                    ? options.series[bucket][x]
                    : [];
            ...
            const percentage = self.uConverter.convert(
                (tooltipData.length / options.heatmap.nseries) * 100,
                '',
                '',
                { unit: '', precision: precision },
            );
            ttData.affectedSeries =
                percentage +
                '% of Series, ' +
                tooltipData.length +
                ' of ' +
                options.heatmap.nseries;
            ttData.bucketRange = [range[0], range[1]];
            /* eslint-disable max-len */
            ttData.color = !Array.isArray(options.heatmap.color)
                ? options.heatmap.color
                : options.heatmap.colorValueMap[tooltipData.length];
            ...
            for (let i = 0; i < n; i++) {
                ...
                const val = self.uConverter.convert(
                    tooltipData[i].v,
                    format.unit,
                    dunit,
                    { unit: format.unit, precision: precision },
                ...
                const tagData: any = {
                    label: tooltipData[i].label,
                    value: val,
                    tags: tooltipData[i].tags,
                };
            ...
            self.ttDataSvc._ttDataPut({
                data: ttData,
                position: { x: event.x, y: event.y, plotArea },
            });
        };
        ...
        if (!changes) {
                    ...
                    if (k === 'y' || k === 'y2') {
                        axis.pixelsPerLabel =
                            this.size.height <= 250
                                ? 6 * Math.ceil(this.size.height / 50)
                                : 50;
                    }
                    if (axis.tickFormat) {
            ...
            if (
                ...
                if (this.chartType === 'line') {
                    ...
                    this.options.zoomCallback = function (
                        minDate,
                        maxDate,
                        yRanges,
                    ) {
                        ...
                        if (!yRanges && n > 0) {
                            const actualStart =
                                new Date(self.data.ts[0][0]).getTime() / 1000;
                            const actualEnd =
                                new Date(self.data.ts[n - 1][0]).getTime() /
                                1000;
                    ...
                    this.options.interactionModel.dblclick = function (
                        e,
                        g,
                        context,
                    ) {
                        if (g.user_attrs_.isCustomZoomed) {
                            ...
                            g.axes_.forEach((axis, i) => {
                                ...
                                if (axis.valueRange) {
                                    axis.valueRange =
                                        g.user_attrs_.axes[axisKey].valueRange;
                                }
                            });
                            g.drawGraph_();
                        }
                    ...
                    if (this.timeseriesLegend) {
                        ...
                        let clickCount = 0;
                        const handleSingleDoubleClick = function (
                            e,
                            x,
                            points,
                        ) {
                    ...
                    this.options.interactionModel = {
                        mousedown: function (event, g, context) {
                            ...
                            const plotArea = g.layout_.getPlotArea();
                            const height =
                                plotArea.h / g.user_attrs_.heatmap.buckets;
                            const width =
                                g.layout_.points.length > 1
                                    ? g.layout_.points[0][1].canvasx -
                                      g.layout_.points[0][0].canvasx
                                    : 0;
                            ...
                            let xv = g.toDataXCoord(
                                Dygraph.pageX(event) - graphPos.x,
                            );
                            const d = xv % xdiff;
                            ...
                            if (cx >= plotArea.x && cy <= plotArea.h) {
                                const bucket =
                                    g.user_attrs_.heatmap.buckets -
                                    (cy - (cy % height)) / height;
                                const ts = g.toDataXCoord(cx2);
                                const hasData =
                                    g.user_attrs_.series[bucket] &&
                                    g.user_attrs_.series[bucket][ts];
                        ...
                        if (this.chartType === 'line') {
                            ...
                            this.currentTickEvent.emit({
                                ...
                                tickData: {
                                    ...
                                    bucket: 1,
                                    data:
                                        this._g.user_attrs_.series[1] &&
                                        this._g.user_attrs_.series[1][ts],
                                },
                            });
                        }
                    }
                ...
                this._g &&
                (changes.eventBuckets || changes.showEvents)
            ) {
        ...
        if (this.firstTickHighlight && this.labelsDiv) {
            ...
            if (parent) {
                widgetScrollContainer =
                    parent.querySelector('.multigraph-output');
            } else {
                // no widget-output (probably not a dashboard widget)
                // so lets check for graph-output in alerts
            ...
            if (
                winSize.width - event.x <
                tooltipSize.width + offsetAmount * 2
            ) {
                // left
            ...
            if (
                winSize.height - event.y <
                tooltipSize.height + offsetAmount * 2
            ) {
                // above
