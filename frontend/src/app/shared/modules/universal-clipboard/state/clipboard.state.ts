...
export class ClipboardCreate {
    ...
    constructor(public readonly title: any) {}
}
...
export class ClipboardRemove {
    ...
    constructor(public readonly index: any) {}
}
...
export class UniversalClipboardState {
    ...
    @Selector() static getClipboardResourceLoaded(
        state: ClipboardResourceModel,
    ) {
        return state.loaded;
    }
    ...
    @Selector() static getActiveClipboardSelected(
        state: ClipboardResourceModel,
    ) {
    ...
    @Selector() static getActiveClipboardSelectedIndex(
        state: ClipboardResourceModel,
    ) {
    ...
    clipboardResourceInitialization(
        ctx: StateContext<ClipboardResourceModel>,
        ...
        if (!cbResource) {
            ...
            // user folder
            const userFolder = this.store.selectSnapshot(
                DbfsResourcesState.getFolder('/user/' + user.alias),
            );
            ...
            this.store
                .dispatch(new DbfsCreateFolder(folderPayload, {}))
                .pipe(
                    map((res: any) => {
                        // folder created, now lets create default clipboard
                        this.service
                            .createClipboardResource('Default clipboard')
                            .pipe(
                                map((payload: any) => {
                                    ...
                                    this.store
                                        ...
                                        .pipe(
                                            map(() => {
                                                ...
                                                this.store.dispatch(
                                                    new ClipboardResourceInitialize(),
                                                );
                                            }),
                                        )
                                        .subscribe();
                                ...
                                catchError((error) =>
                                    ctx.dispatch(
                                        new ClipboardError(
                                            error,
                                            'clipboardResourceInitialization :: Create default clipboard resource [1]',
                                        ),
                                    ),
                                ),
                            );
                    ...
                    catchError((error) =>
                        ctx.dispatch(
                            new ClipboardError(
                                error,
                                'clipboardResourceInitialization :: Create clipboard folder',
                            ),
                        ),
                    ),
                )
            ...
            this.store
                .dispatch(new DbfsLoadSubfolder(cbResource.fullPath, {}))
                .pipe(
                    map(() => {
                        // check for default clipboard
                        const defaultCheck = this.store.selectSnapshot(
                            DbfsResourcesState.getFile(
                                cbResource.fullPath + '/default-clipboard',
                            ),
                        );
                        if (!defaultCheck) {
                                ...
                                .pipe(
                                    ...
                                    catchError((error) =>
                                        ctx.dispatch(
                                            new ClipboardError(
                                                error,
                                                'clipboardResourceInitialization :: Create default clipboard resource [2]',
                                            ),
                                        ),
                                    ),
                                )
                    ...
                    catchError((error) =>
                        ctx.dispatch(
                            new ClipboardError(
                                error,
                                'clipboardResourceInitialization :: Loading resource folder',
                            ),
                        ),
                    ),
                )
    ...
    clipboardResourceInitializationSuccess(
        ctx: StateContext<ClipboardResourceModel>,
        {}: ClipboardResourceInitializeSuccess,
    ) {
        // get resource
        const resource: DbfsFolderModel =
            this.service.clipboardFolderResource();
        ...
        const curActiveIndex = this.store.selectSnapshot(
            UniversalClipboardState.getActiveClipboardSelectedIndex,
        );
    ...
    clipboardLoad(
        ...
        const state = ctx.getState();
        const active: any[] = state.clipboardsList.filter(
            (item) => item.active,
        );
        ...
        if (active.length === 0) {
            ctx.dispatch(
                new ClipboardError(
                    new Error('No active clipboard'),
                    'Clipboard Load',
                ),
            );
        } else {
            this.service.loadClipboard(active[0].resource.id).subscribe(
                (res: any) => {
                    ctx.dispatch(new ClipboardLoadSuccess(res.body));
                },
                (error) => {
                    ctx.dispatch(new ClipboardError(error, 'Clipboard Load'));
                },
            );
        }
    }
    ...
    clipboardLoadSuccess(
        ctx: StateContext<ClipboardResourceModel>,
        { response }: ClipboardLoadSuccess,
    ) {
        const state = ctx.getState();

        for (let i = 0; i < response.content.widgets.length; i++) {
            if (!response.content.widgets[i].settings.clipboardMeta.cbId) {
                response.content.widgets[i].settings.clipboardMeta.cbId =
                    this.service.generateUniqueClipboardItemId(
                        response.content.widgets[i].id,
                    );
            }
        }
    ...
    clipboardCreate(
        ctx: StateContext<ClipboardResourceModel>,
        { title }: ClipboardCreate,
    ) {
        this.service.createClipboardResource(title).subscribe(
            (res: any) => {
                ctx.dispatch(new ClipboardCreateSuccess(res.body));
            },
            (error) => {
                ctx.dispatch(
                    new ClipboardError(error, 'Create Clipboard failed'),
                );
            },
        );
    }
    ...
    clipboardCreateSuccess(
        ctx: StateContext<ClipboardResourceModel>,
        { response }: ClipboardCreateSuccess,
    ) {
        const cbResource = this.service.clipboardFolderResource();
        this.store
            .dispatch(new DbfsLoadSubfolder(cbResource.fullPath, {}))
            .pipe(
                map(() => {
                    ...
                    ctx.dispatch(
                        ...
                        const state = ctx.getState();
                        const list = state.clipboardsList;
                        const index = list.findIndex(
                            (item) => item.resource.id === response.id,
                        );
                        ...
                        if (index > -1) {
                            ...
                            throw new Error(
                                'Can not find index for new clipboard',
                            );
                        }
                    });
                }),
                catchError((error) =>
                    ctx.dispatch(
                        new ClipboardError(
                            error,
                            'clipboardCreateSuccess :: error in clipboard creation',
                        ),
                    ),
                ),
            )
    ...
    clipboardModify(
        ctx: StateContext<ClipboardResourceModel>,
        { clipboard, index }: ClipboardModify,
    ) {
        // TODO
    }

    @Action(ClipboardModifySuccess)
    clipboardModifySuccess(
        ctx: StateContext<ClipboardResourceModel>,
        { response, index }: ClipboardModifySuccess,
    ) {
        // TODO
    }
    ...
    clipboardRemove(
        ...
        if (index === 0) {
            ...
            ctx.dispatch(
                new ClipboardError(
                    { message: 'Can not delete Default' },
                    'clipboardRemove :: error in clipboard removal',
                ),
            );
        } else {
            const cbList = state.clipboardsList;
            // get the cb "dashboard"
            const cbDB = cbList[index];
            ...
            ctx.dispatch([
                ...
                (error: any) => {
                    ...
                    ctx.dispatch(
                        new ClipboardError(
                            error,
                            'Clipboard Removal [Clipboard Remove]',
                        ),
                    );
                },
            );
    ...
    clipboardRemoveSuccess(
        ctx: StateContext<ClipboardResourceModel>,
        { response, index }: ClipboardRemoveSuccess,
    ) {
    ...
    clipboardAddItems(
            ...
            ctx.dispatch(new ClipboardLoad()).subscribe(
                ...
                (error: any) => {
                    ctx.dispatch(
                        new ClipboardError(
                            error,
                            'Clipboard Add Items [Clipboard Load] - FINAL TRY',
                        ),
                    );
                },
            );
        ...
        const clipboard: any = this.utils.deepClone(state.clipboard);
        ...
        if (!clipboard || !clipboard.content) {
            ...
            for (let i = 0; i < items.length; i++) {
                ...
                items[i].settings.clipboardMeta.cbId =
                    this.service.generateUniqueClipboardItemId(items[i].id);
            }

            const mergedWidgets: any[] = items.concat(
                clipboard.content.widgets || [],
            );
            ...
            this.service.saveClipboard(activeId, clipboard).subscribe(
                (res: any) => {
                    ctx.dispatch(new ClipboardAddItemsSuccess(res, {}));
                },
                (error) => {
                    ctx.dispatch(
                        new ClipboardError(error, 'Save Clipboard Failed'),
                    );
                },
            );
        }
    ...
    clipboardAddItemsSuccess(
        ctx: StateContext<ClipboardResourceModel>,
        { response, args }: ClipboardAddItemsSuccess,
    ) {
        const state = ctx.getState();
    ...
    clipboardModifyItem(
        ctx: StateContext<ClipboardResourceModel>,
        { item, index }: ClipboardModifyItem,
    ) {
        // TODO
    }

    @Action(ClipboardModifyItemSuccess)
    clipboardModifyItemSuccess(
        ctx: StateContext<ClipboardResourceModel>,
        { response, index }: ClipboardModifyItemSuccess,
    ) {
        // TODO
    }
    ...
    clipboardRemoveItems(
        ...
        widgets = widgets.filter(
            (item: any) => !items.includes(item.settings.clipboardMeta.cbId),
        );

        dashboard.content.widgets = widgets;

        // now need to save dashboard
        this.http.saveDashboard(dashboard.id, dashboard).subscribe(
            (res: any) => {
                ctx.dispatch(new ClipboardRemoveItemsSuccess(dashboard));
            },
            (error) => {
                ctx.dispatch(
                    new ClipboardError(error, 'Remove Clipboard Items Failed'),
                );
            },
        );
    }
    ...
    clipboardRemoveItemSuccess(
        ctx: StateContext<ClipboardResourceModel>,
        { response }: ClipboardRemoveItemsSuccess,
    ) {
        const state = ctx.getState();

        ctx.setState({ ...state, clipboard: response });
    }
    ...
    setClipboardActive(
        ctx: StateContext<ClipboardResourceModel>,
        { index }: SetClipboardActive,
    ) {
        const state = ctx.getState();
        const clipboardsList: any[] = this.utils.deepClone(
            state.clipboardsList,
        );
        ...
        try {
            ...
            ctx.dispatch(
                new ClipboardError(error, 'Set clipboard active Failed'),
            );
        }
