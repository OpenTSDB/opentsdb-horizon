...
export class ChartjsDirective implements OnInit, OnChanges, OnDestroy {
    ...
    constructor(
        ...
        const tooltipFormatter2 = function (item, data) {
            ...
            let value: any;
            const color: any =
                data.datasets[datasetIndex].backgroundColor[index];
            const label: any = item.label ? item.label : tags.metric;
            if (
                self.options.scales &&
                ...
                cvalue = self.uConverter.convert(value, unit, dunit, {
                    unit: unit,
                    precision: tickFormat.precision
                });
            } else {
                ...
                cvalue = self.uConverter.convert(value, '', dunit, {
                    unit: '',
                    precision: ''
                });
            }
        ...
        if (changes) {
            ...
            const tickFormatter = function (value) {
                if (this.options.ticks && this.options.ticks.format) {
                    ...
                    return self.uConverter.convert(value, unit, dunit, {
                        unit: unit,
                        precision: this.options.ticks.format.precision
                    });
                } else {
                    return value;
                }
            ...
            if (
                this.chart &&
                (changes.chartType || changes.data || changes.options)
            ) {
                this.chart.destroy();
    ...
    updateDatasets(datasets) {
        ...
        this.max =
            datasets[0] && datasets[0].data ? Math.max(...datasets[0].data) : 0;
        const multiColor =
            (this.chartType === 'bar' || this.chartType === 'doughnut') &&
            datasets.length === 1
                ? true
                : false;
    ...
    getRandomColor(color) {
        ...
        if (color) {
            ...
            // TODO: figure out how to do this without disabling eslint
            // NOTE: disabling eslint for these lines for now
