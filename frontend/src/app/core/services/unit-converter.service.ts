...
export class UnitConverterService {
    ...
    format(value, options) {
        ...
        oUnit =
            !oUnit || options.unit === 'usd'
                ? this.getDefaultUnit(value, true)
                : oUnit;
        ...
        precision =
            Number.isInteger(result) && !options.precisionStrict
                ? 0
                : precision;
        result = sign * result;
    ...
    getNormalizedUnit(value, options) {
        ...
        sUnit =
            !sUnit || options.unit === 'usd'
                ? this.getDefaultUnit(value)
                : sUnit;
        ...
        if (sUnit.type === 'time') {
            switch (sUnit.key) {
                ...
                case 'microseconds':
                    ...
                    break;
                default:
                    const timeUnits: any[] = this.units.filter(
                        (d: any) => d.type === 'time',
                    );
                    for (let i = sUnit.index; i < timeUnits.length; i++) {
                        ...
                        const dstMethodName =
                            'as' +
                            timeUnits[i].key[0].toUpperCase() +
                            timeUnits[i].key.substr(1);
                        const newValue = moment
                            .duration(value, sUnit.key)
                            [dstMethodName]();
            ...
            const units: any[] = this.units.filter(
                (d: any) => d.type === sUnit.type,
            );
    ...
    convert(value, srcUnitKey, destUnitKey, options): any {
        ...
        let precision = options.precision
            ...
            : options.precision === 'auto' || options.precision === ''
                ? 2
                : 0;
        precision = precision > 100 ? 100 : precision; // max number supported by toFixed() func.
        const prefix = options.unit === 'usd' ? '$' : '';
        let postfix =
            options.unit && this.isCustomUnit(options.unit) ? options.unit : ''; // unknown units will be added as postfix
            ...
            if (dUnit && dUnit.type === 'time') {
                switch (srcUnitKey) {
                    ...
                    default:
                        const dstMethodName =
                            'as' +
                            dUnit.key[0].toUpperCase() +
                            dUnit.key.substr(1);
                        result = moment
                            .duration(value, srcUnitKey)
                            [dstMethodName]();
                }
            } else if (dUnit) {
        ...
        if (
            (options.precision === 'auto' || options.precision === '') &&
            result &&
            !isNaN(parseInt(result, 10))
        ) {
            const sVal = value.toString();
            const nzeros =
                sVal.indexOf('.') !== -1
                    ? sVal.match(/(\.0*)/)[0].length - 1
                    : 0;
            precision = nzeros + 2;
        }
        ...
        if (options.detail) {
            ...
            postfix =
                this.isCustomUnit(options.unit) || options.unit === 'usd'
                    ? dUnit.unit
                    : '';
            return {
                num: parseFloat(result.toFixed(precision)),
                postfix: postfix,
                unit: this.isCustomUnit(options.unit)
                    ? options.unit
                    : options.unit === 'usd'
                        ? oUnit.unit
                        : dUnit.unit,
    ...
    getDefaultUnit(value, bigUnit = false) {
        ...
        const power =
            value === 0 ? 0 : Math.floor(Math.log(value) / Math.log(1000));
        const defaultUnits: any[] = this.units.filter(
            (d: any) => d.type === 'default',
        );
    ...
    getNoramilizedValue(value, options) {
        ...
        const res = this.convert(value, options.unit, destUnit, {
            ...options,
            detail: true,
        });
        return res;
