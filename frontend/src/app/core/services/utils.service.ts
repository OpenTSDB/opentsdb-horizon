...
export class UtilsService {
        ...
        for (const metric of metrics) {
            if (metric.expression) {
                if (metric.expression.indexOf('{{' + oldID + '}}') !== -1) {
                    metric.expression = metric.expression.replace(
                        idreg,
                        '{{' + newID + '}}',
                    );
                }
    ...
    getWidgetQueryStatistics(queries) {
        ...
        for (let i = 0; i < queries.length; i++) {
            ...
            for (let j = 0; j < queries[i].metrics.length; j++) {
                ...
                if (
                    !queries[i].metrics[j].settings.visual ||
                    queries[i].metrics[j].settings.visual.visible === true
                ) {
                    metricsVisibleLen++;
                    /* eslint-disable max-len */
                    if (
                        !queries[i].metrics[j].settings.visual ||
                        queries[i].metrics[j].settings.visual.color ===
                            'auto' ||
                        !queries[i].metrics[j].settings.visual.color
                    ) {
    ...
    setWidgetTimeConfiguration(widget, config) {
        widget.settings.time = {
            ...
            downsample: {
                ...
                customValue:
                    config.downsample !== 'custom'
                        ? ''
                        : config.customDownsampleValue,
                customUnit:
                    config.downsample !== 'custom'
                        ? ''
                        : config.customDownsampleUnit,
    ...
    updateQuery(widget, payload) {
        ...
        const qindex = query.id
            ? widget.queries.findIndex((q) => q.id === query.id)
            : -1;
        if (qindex === -1) {
    ...
    updateQueryVisual(widget, qid, mid, visual, ex = []) {
        ...
        for (let i = 0; i < widget.queries[qindex].metrics.length; i++) {
            ...
            if (!ex.length || !ex.includes(id)) {
                widget.queries[qindex].metrics[i].settings.visual = {
                    ...widget.queries[qindex].metrics[i].settings.visual,
                    ...visual,
                };
            }
    ...
    updateQueryMetricVisual(widget, qid, mid, visual) {
        ...
        const qindex = widget.queries.findIndex((d) => d.id === qid);
        const mindex = widget.queries[qindex].metrics.findIndex(
            (d) => d.id === mid,
        );
        widget.queries[qindex].metrics[mindex].settings.visual = {
            ...widget.queries[qindex].metrics[mindex].settings.visual,
            ...visual,
        };
    }
    ...
    getWidgetMetricDefaultLabel(queries, qIndex, mIndex: number) {
        ...
        for (let i = 0; i < queries[qIndex].metrics.length; i++) {
            const isExpression =
                queries[qIndex].metrics[i].expression !== undefined;
            if (!isExpression) {
                m++;
            } else {
                e++;
            ...
            if (i === mIndex) {
                // mIndex is string here
                break;
            }
        }
        let label =
            queries.length > 1 ? 'Q' + (parseInt(qIndex, 10) + 1) + ':' : '';
    ...
    tagMacro(metric: any, inputString: string): string {
        ...
        if (matches) {
            ...
            for (let i = 0; i < matches.length; i++) {
                ...
                const keyword = captureGroupSplit[0]
                    .substring(2)
                    .toLowerCase()
                    .trim();
                const tagKey = captureGroupSplit[1]
                    .substring(0, captureGroupSplit[1].length - 2)
                    .toLowerCase()
                    .trim();
                ...
                for (const [_captureGroup, tagValue] of Object.entries(
                    captureGroupToValueMap,
                )) {
    ...
    getColorsHSV(color = null, n = 1, shades = null) {
        ...
        if (shades && shades === 'dark' && contrast.type === 'black') {
            // light theme => dark shades +
            srange[1] = 0.4;
        } else if (shades && shades === 'light' && contrast.type === 'white') {
            // dark theme => light shades + light color
            srange[0] = 0.6;
        }

        // no. of colors on light to bright (sBand) and no. of bright to dark (vBand)
        const sBand = Math.ceil(n * 1),
            vBand = Math.floor(n * 0);
    ...
    */
    ...
    arrayUnique(items) {
        items = items.filter((elem, pos, arr) => arr.indexOf(elem) === pos);
        return items;
    }
            ...
            for (let j = 0; j < queries[i].metrics.length; j++) {
                ...
                if (queries[i].metrics[j].id === metricId) {
                    if (queries[i].metrics[j].expression) {
                        ...
                        return {
                            id: 'q' + queryIndex + '_' + 'e' + expressionIndex,
                            qIndex: queryIndex - 1,
                            mIndex: metricIndex - 1,
                            expression: true,
                        };
                    } else {
                        // eslint-disable-next-line max-len
                        return {
                            id: 'q' + queryIndex + '_' + 'm' + metricIndex,
                            qIndex: queryIndex - 1,
                            mIndex: metricIndex - 1,
                            expression: false,
                        };
    ...
    getQueryClone(queries, index) {
        ...
        const oldIds = this.getIDs(query.metrics),
            newIds = [];
                ...
                switch (fxCall) {
                    case 'Timeshift':
                        ...
                        const timeAmount = parseInt(
                            functions[i].val.match(timeAmountRegEx)[0],
                            10,
                        );
                        const timeUnit = functions[i].val
                            .match(timeUnitRegEx)[0]
                            .toLowerCase();
                        if (timeUnit === 'w') {
    ...
    getEventBuckets(startTime, endTime, maxNumOfBuckets, events) {
        ...
        for (let i = validBucketSizes.length - 1; i >= 0; i--) {
            const numOfBuckets = parseInt(
                (duration / validBucketSizes[i]).toString(),
                10,
            );
        ...
        buckets.push({
            ...
            width: bucketSize * minuteAsMilliseconds,
            events: [],
        });

        // create remaining buckets
        while (
            bucketEndTime + 2 * bucketSize * minuteAsMilliseconds <
            endTime
        ) {
        ...
        for (const event of events) {
            ...
            for (const bucket of buckets) {
                if (
                    eStartTime >= bucket.startTime &&
                    eStartTime < bucket.endTime
                ) {
    ...
    getTimeArray(timeInMilliseconds, timezone?: string): any[] {
        ...
        const month =
            timezone === 'utc' ? months[a.getUTCMonth()] : months[a.getMonth()];
    ...
    buildDisplayTime(
        ...
        let dateString = '';
        if (crossedMidnight) {
            // include full time, including year and date
            dateString =
                time[0] +
                '-' +
                time[1] +
    ...
    isNumber(value: string | number): boolean {
        if (value === 0) {
            ...
            return value ? !Number.isNaN(parseInt(value.toString(), 10)) : false;
        }
    }
            ...
            if (ftype === 'Chain' && filter['op'] === 'OR') {
                ...
                switch (ftype) {
                    case 'TagValueLiteralOr':
                        ...
                        break;
                    case 'TagValueRegex':
                        values = ['regexp(' + iFilter.filter + ')'];
                        break;
                }
                const index = newFilters.findIndex(
                    (d) => d.tagk === iFilter.tagKey,
                );
                if (index === -1) {
                    newFilters.push({ tagk: iFilter.tagKey, filter: values });
                } else {
                    newFilters[index].filter =
                        newFilters[index].filter.concat(values);
                }
            }
        }
        return newFilters;
    ...
    getExpressionLabel(qindex, mindex, queries) {
        ...
        for (
            let i = 0;
            i <= mindex && i < queries[qindex].metrics.length;
            i++
        ) {
            if (queries[qindex].metrics[i].expression) {
                eIndex++;
