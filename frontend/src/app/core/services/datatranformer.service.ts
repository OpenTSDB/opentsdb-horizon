...
export class DatatranformerService {
    ...
    openTSDBToTable(widget, options, result: any): any {
        ...
        const summary =
            visual.layout === 'metrics:tags' ||
            visual.layout === 'tags:metrics';
        const decimals =
            !visual.decimals || visual.decimals.toString().trim() === 'auto'
                ? 2
                : visual.decimals;
        ...
        if (result && result.results) {
            ...
            for (let i = 0; i < result.results.length; i++) {
                ...
                const mIndex = this.util.getDSIndexToMetricIndex(
                    widget.queries[qIndex],
                    parseInt(qids[3], 10) - 1,
                    qids[2],
                );
                ...
                const gConfig = widget.queries[qIndex]
                    ...
                    : null;
                const mConfig =
                    gConfig && gConfig.metrics[mIndex]
                        ? widget.queries[qIndex].metrics[mIndex]
                        : null;
                const vConfig =
                    mConfig && mConfig.settings ? mConfig.settings.visual : {};
                if (
                    (source === 'summarizer' && !summary) ||
                    (source !== 'summarizer' && summary) ||
                    !gConfig ||
                    !gConfig.settings.visual.visible ||
                    !vConfig.visible
                ) {
                    continue;
                }
                ...
                for (let j = 0; j < n; j++) {
                    let data = summary
                        ? queryResults.data[j].NumericSummaryType
                        : queryResults.data[j].NumericType;
                    const tags = this.util.sortObject(
                        queryResults.data[j].tags,
                    );
                    const mLabel = this.util.getWidgetMetricDefaultLabel(
                        widget.queries,
                        qIndex,
                        mIndex,
                    );
                    const metric = !mConfig.expression
                        ? queryResults.data[j].metric
                        : mLabel;
                    let label = vConfig.label ? vConfig.label : metric;

                    const tagLabel =
                        this.getLableFromMetricTags('', {
                            metric: '',
                            ...tags,
                        }) || 'All';
                    ...
                    if (summary) {
                        label = this.getLableFromMetricTags(label, {
                            metric: metric,
                            ...tags,
                        });
                        const colId =
                            visual.layout === 'metrics:tags' ? tagLabel : mLabel;
                        const colLabel =
                            visual.layout === 'metrics:tags' ? tagLabel : label;
                        const rowId =
                            visual.layout === 'metrics:tags' ? mLabel : tagLabel;
                        displayColumns[colId] = {
                            id: colId,
                            label: colLabel,
                            shortLabel:
                                colLabel.length > colLabelLen
                                    ? colLabel.substr(0, colLabelLen - 2) + '..'
                                    : colLabel,
                        };
                        if (!objData[rowId]) {
                            objData[rowId] =
                        ...
                        const dunit = this.unit.getNormalizedUnit(
                            data[ts][index],
                            format,
                        );

                        objData[rowId][colId] = this.unit.convert(
                            data[ts][index],
                            format.unit,
                            dunit,
                            format,
                        ...
                        label = this.getLableFromMetricTags(vConfig.label, {
                            metric: metric,
                            ...tags,
                        });
                        displayColumns[id] = {
                            ...
                            label: label,
                            shortLabel:
                                label.length > colLabelLen
                                    ? label.substr(0, colLabelLen - 2) + '..'
                                    : label,
                        };
                        objData[id] = {
                            rawdata: data,
            ...
            if (summary) {
                ...
                const numPoints = keys.length
                    ? objData[keys[0]].data.length
                    : 0;
                const dtFormat = dtFormats[unit] || defDtFormat;
                for (let i = 0; i < numPoints; i++) {
                    const time =
                        timeSpecification.start + m * i * mSeconds[unit];
                    const row = {
                        time:
                            options.timezone === 'local'
                                ? moment.unix(time).format(dtFormat)
                                : moment.unix(time).utc().format(dtFormat),
                        'time:raw': time,
                    };
                    for (let j = 0; j < keys.length; j++) {
                        ...
                        const bgColor = !isNaN(objData[key].rawdata[i])
                            ? this.overrideColor(
                                objData[key].rawdata[i],
                                objData[key].config.color,
                                objData[key].config.conditions,
                            )
                            : '';
                        const rgbColor = bgColor ? d3.rgb(bgColor) : '';
                        const color = rgbColor
                            ? 'rgb(' +
                              Math.floor(255 - rgbColor.r) +
                              ',' +
                              Math.floor(255 - rgbColor.g) +
                              ',' +
                              Math.floor(255 - rgbColor.b) +
                              ')'
                            : '';
        ...
        options.displayColumns.sort((a: any, b: any) => this.util.sortAlphaNum(a.label, b.label));
        options.displayColumns.unshift(
            !summary
                ? { id: 'time', shortLabel: 'Time' }
                : visual.layout === 'metrics:tags'
                    ? { id: 'metric', shortLabel: 'Metric' }
                    : { id: 'tag', shortLabel: 'Tag' },
        );
        return dataTable;
    }
        ...
        if (result !== undefined && result.results) {
            ...
            for (let i = 0; i < result.results.length; i++) {
                ...
                if (source === 'summarizer') {
                    ...
                    for (let j = 0; j < n; j++) {
                        ...
                        const hash = this.getHashFromMetricAndTags(
                            result.results[i].data[j].metric,
                            tags,
                        );
                        const aggs =
                            result.results[i].data[j].NumericSummaryType
                                .aggregations;
            ...
            if (Object.keys(removeSource).length) {
                for (let i = result.results.length - 1; i >= 0; i--) {
                    ...
                    if (removeSource[mid]) {
                        const n = result.results[i].data.length;
                        result.results[i].data = result.results[i].data.filter(
                            (d) => {
                                const hash = this.getHashFromMetricAndTags(
                                    d.metric,
                                    d.tags,
    ...
    openTSDBToDygraph(
        ...
        for (let i = 0; i < result.results.length; i++) {
            ...
            const mConfig =
                gConfig && gConfig.metrics[mIndex]
                    ? gConfig.metrics[mIndex]
                    : null;
            const vConfig =
                mConfig && mConfig.settings ? mConfig.settings.visual : {};
            queryResultsObj[i] = Object.assign({}, queryResultsObj[i], {
                visualType: vConfig.type || 'line',
                mid,
            });
            ...
            if (gConfig && gConfig.settings.visual.visible && vConfig.visible) {
                ...
                if (source === 'summarizer') {
                    ...
                    for (let j = 0; j < n; j++) {
                        ...
                        const aggs =
                            queryResultsObj[i].data[j].NumericSummaryType
                                .aggregations;
                        const key = Object.keys(
                            queryResultsObj[i].data[j].NumericSummaryType
                                .data[0],
                        ...
                        if (!vConfig.axis || vConfig.axis === 'y1') {
                            yMax =
                                yMax < aggData['max'] ? aggData['max'] : yMax;
                            min['y1'] =
                                min['y1'] === null || min['y1'] > aggData['min']
                                    ? aggData['min']
                                    : min['y1'];
                        } else {
                            y2Max =
                                y2Max < aggData['max'] ? aggData['max'] : y2Max;
                            min['y2'] =
                                min['y2'] === null || min['y2'] > aggData['min']
                                    ? aggData['min']
                                    : min['y2'];
                        }
                    }
                    ...
                    if (
                        !vConfig.color &&
                        (!vConfig.scheme || vConfig.scheme === 'auto')
                    ) {
                        nAutoColors += n;
                    }
                    totalSeries += n;
                    ...
                    const key =
                        queryResultsObj[i].timeSpecification.start +
                        '-' +
                        queryResultsObj[i].timeSpecification.end +
                        '-' +
                        queryResultsObj[i].timeSpecification.interval;
                    mTimeConfigs[key] = {
                        timeSpecification: queryResultsObj[i].timeSpecification,
                        nSeries: n,
                    };
                    for (let j = 0; j < n; j++) {
                        const tags = queryResultsObj[i].data[j].tags;
                        const hash = JSON.stringify(tags);
                        dict[mid]['values'][hash] =
                            queryResultsObj[i].data[j].NumericType;
                        if (
                            !hasToT &&
                            ((vConfig.type === 'area' &&
                                vConfig.stacked !== 'false') ||
                                vConfig.type === 'bar')
                        ) {
                            isStacked = true;
                            const axis =
                                !vConfig.axis || vConfig.axis === 'y1'
                                    ? 'y'
                                    : 'y2';
        ...
        const tsObj = this.util.getTimestampsFromTimeSpecification(
            Object.values(mTimeConfigs),
        );
        ...
        for (let i = 0; i < queryResults.length; i++) {
            ...
            const gConfig = widget.queries[qIndex]
                ...
                : null;
            const mConfig =
                gConfig && widget.queries[qIndex].metrics[mIndex]
                    ? widget.queries[qIndex].metrics[mIndex]
                    : {};
            const vConfig =
                mConfig && mConfig.settings
                    ? this.util.deepClone(mConfig.settings.visual)
                    : {};
            ...
            if (vConfig.color && vConfig.color !== 'auto') {
                colors[midExToT] =
                    midExToTNSeries[midExToT] === 1
                        ? [vConfig.color]
                        : this.util
                            .getColorsHSV(
                                vConfig.color,
                                midExToTNSeries[midExToT],
                                options.theme === 'dark' ? 'light' : 'dark',
                            )
                            .reverse();
            } else if (vConfig.scheme && vConfig.scheme !== 'auto') {
                colors[midExToT] = this.util.getColorsFromScheme(
                    vConfig.scheme,
                    midExToTNSeries[midExToT],
                );
                schemeMeta[mid] = true;
            ...
            for (let j = 0; j < n; j++) {
                ...
                const metric = vConfig.label
                    ? vConfig.label
                    : mConfig.expression
                        ? mLabel
                        : queryResults[i].data[j].metric;
                if (
                    gConfig &&
                    gConfig.settings.visual.visible &&
                    vConfig.visible
                    ...
                    const config: any = {
                        strokeWidth: vConfig.lineWeight
                            ? parseFloat(vConfig.lineWeight)
                            : 1,
                        ...
                        isStacked:
                            ...
                            vConfig.stacked !== 'false'
                                ...
                                : false,
                        axis:
                            !vConfig.axis || vConfig.axis === 'y1' ? 'y' : 'y2',
                        metric: metric,
                        tags: {
                        ...
                        stackOrderBy:
                            vConfig.type === 'line'
                                ? 'label'
                                : vConfig.stackOrderBy || 'min',
                        stackOrder: vConfig.stackOrder || 'asc',
                        connectMissingData:
                            vConfig.connectMissingData === 'true'
                                ? true
                                : false,
                    };
                    if (vConfig.type === 'bar') {
                    ...
                    dseries.push({
                        ...
                        config: config,
                        data: data,
                        hash: this.getHashFromMetricAndTags(
                            mConfig.expression
                                ? mLabel
                                : queryResults[i].data[j].metric,
                            tags,
        ...
        dseries.sort((a: any, b: any) =>
            ...
            (
                ...
                a.config.order1.localeCompare(b.config.order1, 'en', {
                    numeric: true,
                    sensitivity: 'base',
                }) ||
                (a.config.group === 'line'
                    ? !hasToT
                        ...
                        : b.config.label.localeCompare(a.config.label) // ToT: current week to be rendered last
                    : // the order is reverse as the area/bar plotter draws series from last to first
                    a.config.aggregations
                        ? a.config.group === 'line' ||
                        (a.config.group !== 'line' &&
                            a.config.stackOrder === 'desc')
                            ? a.config.aggregations[a.config.stackOrderBy] -
                            b.config.aggregations[b.config.stackOrderBy]
                            : b.config.aggregations[b.config.stackOrderBy] -
                            a.config.aggregations[a.config.stackOrderBy]
                        : 0)
            )
        ...
        for (let i = 0; i < dseries.length; i++) {
            ...
            if (options.visibilityHash[dseries[i].hash] !== undefined) {
                options.visibility.push(
                    options.visibilityHash[dseries[i].hash],
                );
            } else {
            ...
            options.series[label].color = colors[midExToT]
                ? colors[midExToT].pop()
                : autoColors[cIndex++ % nAutoColors];
            options.series[label].hash = dseries[i].hash;
            const seriesIndex = options.labels.indexOf(label);
            const axis = dseries[i].config.axis;
            const unit = dseries[i].timeSpecification.interval.replace(
                /[0-9]/g,
                '',
            );
            ...
            for (let k = 0; k < numPoints; k++) {
                const secs =
                    dseries[i].timeSpecification.start + m * k * mSeconds[unit];
                const ms = secs * 1000;
                const tsIndex = tsObj[ms];
                if (tsIndex !== undefined) {
                    normalizedData[tsIndex][seriesIndex] = !isNaN(data[k])
                        ? data[k]
                        : dseries[i].config.connectMissingData
                            ? null
                            : NaN;
                }
                if (
                    isStacked &&
                    !isNaN(data[k]) &&
                    ((type === 'area' && dseries[i].config.isStacked) ||
                        type === 'bar')
                ) {
                    if (data[k] > 0) {
        ...
        if (isStacked) {
            for (let i = 0; i < stackedYAxes.length; i++) {
                ...
                const axisConfig =
                    widget.settings.axes[stackedYAxes[i] === 'y' ? 'y1' : 'y2'];
                ...
                if (isNaN(parseFloat(axisConfig.min))) {
                    options.axes[axis].valueRange[0] =
                        axisMin < 0
                            ? Math.ceil(axisMin + axisMin * 0.05)
                            : axisConfig.scale === 'logscale'
                                ? null
                                : 0;
                }
            }
        ...
        if (
            ...
            options.axes.y.valueRange[0] >= yMax
        ) {
            options.axes.y.valueRange[0] = null;
        }
        ...
        if (
            ...
            options.axes.y2.valueRange[0] >= y2Max
        ) {
            options.axes.y2.valueRange[0] = null;
        }
    ...
    openTSDBToHeatmap(
        ...
        options.labels = ['x'].concat(
            Array.from(Array(options.heatmap.buckets), (x, index) =>
                (index + 1).toString(),
            ),
        );
        options.heatmap.x = [];
        ...
        if (result && result.results) {
            ...
            for (let i = 0; i < result.results.length; i++) {
                ...
                const n = queryResults.data.length;
                const color = !widget.settings.visual.color
                    ? autoColors[cIndex++]
                    : widget.settings.visual.color;
                ...
                for (let j = 0; j < n; j++) {
                    ...
                    options.heatmap.metric = mConfig.expression
                        ? mLabel
                        : queryResults.data[j].metric;
                    ...
                    const unit = timeSpecification.interval.replace(
                        /[0-9]/g,
                        '',
                    );
                    const m = parseInt(timeSpecification.interval, 10);
                    for (let k = 0; k < numPoints; k++) {
                        const time =
                            (timeSpecification.start + m * k * mSeconds[unit]) *
                            1000;
                        if (!Array.isArray(normalizedData[k])) {
                            normalizedData[k] = Array(
                                options.heatmap.buckets + 1,
                            ).fill(null);
                        ...
                        if (!isNaN(data[k])) {
                            ...
                            options.series[bucket][time].push({
                                label: label,
                                v: data[k],
                                tags: tags,
                            });
        ...
        for (let i = 0; i < normalizedData.length; i++) {
            for (let j = 1; j < normalizedData[i].length; j++) {
                if (
                    normalizedData[i][j] !== null &&
                    !bucketValues.includes(normalizedData[i][j])
                ) {
    ...
    openTSDBToChartJS(
        ...
        switch (chartType) {
            case 'bar':
                return this.getChartJSFormattedDataBar(
                    options,
                    widget,
                    data,
                    queryData,
    ...
    getChartJSFormattedDataBar(options, widget, datasets, queryData, stacked) {
        ...
        for (let i = 0; i < results.length; i++) {
            ...
            const gConfig = widget.queries[qIndex]
                ...
                : null;
            const mConfig =
                widget.queries[qIndex] && widget.queries[qIndex].metrics[mIndex]
                    ? widget.queries[qIndex].metrics[mIndex]
                    : null;
            const vConfig =
                mConfig && mConfig.settings ? mConfig.settings.visual : {};
            if (
                ...
                !mConfig ||
                !gConfig.settings.visual.visible ||
                !mConfig.settings.visual.visible
            ) {
                continue;
            }
            ...
            if (
                ...
                colors[mid] =
                    n === 1
                        ? [vConfig.color]
                        : this.util.getColorsHSV(vConfig.color, n);
            }
            for (let j = 0; j < n; j++) {
                const summarizer = this.getSummarizerOption(
                    widget,
                    qIndex,
                    mIndex,
                );
                ...
                const key = Object.keys(
                    results[i].data[j].NumericSummaryType.data[0],
                )[0];
                const aggData =
                    results[i].data[j].NumericSummaryType.data[0][key];
                const mLabel = this.util.getWidgetMetricDefaultLabel(
                    widget.queries,
                    qIndex,
                    mIndex,
                );
                let label = mConfig.settings.visual.label
                    ? mConfig.settings.visual.label
                    : '';
                const aggrIndex = aggs.indexOf(summarizer);
                label = this.getLableFromMetricTags(label, {
                    metric: !mConfig.expression
                        ? results[i].data[j].metric
                        : mLabel,
                    ...tags,
                });
                dseries.push({
        ...
        dseries.sort((a: any, b: any) => a.value - b.value);
    ...
    getLableFromMetricTags(label, tags, len = 70) {
        ...
        if (matches) {
            for (let i = 0, mlen = matches.length; i < mlen; i++) {
    ...
    openTSDBToD3Donut(options, widget, queryData) {
        ...
        dseries.sort((a: any, b: any) => b.value - a.value);

        for (let i = 0; i < dseries.length; i++) {
            const mid = dseries[i].mid;
            options.data.push({
                label: dseries[i].label,
                value: dseries[i].value,
                color: colors[mid]
                    ? schemeMeta['midScheme'][mid]
                        ? colors[mid].shift()
                        : colors[mid].pop()
    ...
    openTSDBToD3Bar(options, widget, queryData) {
        ...
        for (let i = 0; i < results.length; i++) {
            ...
            const color =
                !widget.settings.visual.color ||
                widget.settings.visual.color === 'auto'
                    ? ''
                    : widget.settings.visual.color;
            for (let j = 0; j < n; j++) {
                const summarizer = this.getSummarizerOption(
                ...
                if (!isNaN(aggData[aggrIndex])) {
                    const o = {
                        label: label,
                        value: aggData[aggrIndex],
                        color: this.overrideColor(
                            aggData[aggrIndex],
                            color,
                            widget.settings.visual.conditions,
                        ),
                        tooltipData: tags,
                    };
                    options.data.push(o);
    ...
    getSummarizerOption(widget: any, qIndex: number, metricIndex: number) {
        ...
        if (widget.queries[qIndex].metrics[metricIndex].summarizer) {
            summarizerOption =
                widget.queries[qIndex].metrics[metricIndex].summarizer;
        } else if (
            widget.settings.time.downsample.aggregators &&
            widget.settings.time.downsample.aggregators[qIndex]
        ) {
            // todo: remove once summarizer exposed for all widgets
            summarizerOption =
                widget.settings.time.downsample.aggregators[qIndex];
        } else {
