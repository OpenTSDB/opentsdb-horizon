    ...
    getQuery(source, type, params, andOp = true) {
        ...
        for (let i = 0, len = params.length; i < len; i++) {
            ...
            query.namespace =
                type !== 'NAMESPACES'
                    ? params[i].namespace ||
                      this.appConfig.getDefaultNamespace()
                    : this.utilsService.convertPatternTSDBCompat(
                        params[i].search,
                    );
            if (type === 'TAG_KEYS_AND_VALUES' && params[i].tagkey) {
                ...
                filters.push({
                    ...
                    filter: librange
                        ? librange[1]
                        : this.utilsService.convertPattern(params[i].search),
            ...
            switch (type) {
                case 'METRICS':
                    filters.push({
                        type: 'MetricRegex',
                        metric: this.utilsService.convertPattern(
                            params[i].search,
                        ),
                    });
                    break;

                // set the metrics filter only if its set. tsdb requires at least one filter in the query
                case 'TAG_KEYS':
                    if (mSource === 'meta') {
                        filters.push({
                            type: 'TagKeyRegex',
                            filter: this.utilsService.convertPattern(
                                params[i].search,
                            ),
                        });
                    }
