...
export class DateUtilsService {
    ...
    strippedRelativeTime(relativeTime: string): string {
        ...
        let timeUnit: string = this.getTimeUnitAbbr(
            relativeTime.split(timeAmount.toString()).pop(),
        );
    ...
    relativeTimeToMoment(relativeTime: string): Moment {
        const timeAmount: number = this.getTimeAmount(relativeTime);
        const timeUnit: string = this.getTimeUnitAbbr(
            relativeTime.split(timeAmount.toString()).pop(),
        );
        ...
        if (
            (timeUnit === this.timeAbbr.year && timeAmount < 1 * numYears) ||
            ...
            (timeUnit === this.timeAbbr.minute &&
                timeAmount < 525600 * numYears) ||
            (timeUnit === this.timeAbbr.second &&
                timeAmount < 525600 * 60 * numYears)
        ) {
    ...
    isRelativeTime(time) {
        return (
            this.relativeTimeToMoment(time) ||
            this.timeToTime(time) ||
            time.toLowerCase() === 'now'
        );
    }
    ...
    timeToMoment(time: string, timezone: string): Moment {
        ...
        if (this.defaultTimeToMoment(time, timezone)) {
            ...
            _moment = moment();
        } else if (this.timeToTime(time)) {
            // e.g., this 'quarter'
            if (timezone === 'utc') {
                _moment = moment
                    .utc()
                    .startOf(this.timeToTime(time.toLowerCase()));
            } else {
            ...
            _moment =
                timezone.toLowerCase() === 'utc'
                    ? moment.utc(time, 'YYYYMMDDTHHmmss', true)
                    : moment(time, 'YYYYMMDDTHHmmss', true);
        } else if (this.isTimeStampValid(time)) {
            // e.g., 1234567890
            _moment = moment.unix(Number(time));
        } else if (this.relativeTimeToMoment(time)) {
            // e.g., 1h
            _moment = this.relativeTimeToMoment(time);
        } else if (this.dateWithoutTimeToMoment(time, timezone)) {
            // e.g., 05/05/18
