...
export class MultigraphService {
    ...
    constructor(
        ...
        private tranxService: DatatranformerService,
    ) {}
    ...
    fillMultiTagValues(widget: any, multiConf: any, rawdata: any): any {
        const isMultiByQuery = this.isMultiQuery(multiConf);
        const xTemp = multiConf.x
            ? '{{{' + Object.keys(multiConf.x).join('}}}/{{{') + '}}}'
            : 'x';
        const yTemp = multiConf.y
            ? '{{{' + Object.keys(multiConf.y).join('}}}/{{{') + '}}}'
            : 'y';
        let xCombine = [];
        let yCombine = [];
        let lookupData = {};
        ...
        for (let i = 0; i < rawdata.results.length; i++) {
            ...
            const mIndex = this.utils.getDSIndexToMetricIndex(
            ...
            const mLabel = this.utils.getWidgetMetricDefaultLabel(
                widget.queries,
                qIndex,
                mIndex,
            );
            const gLabel = gConfig.settings.visual.label
                ? gConfig.settings.visual.label
                : '';
            if (gConfig && gConfig.settings.visual.visible && vConfig.visible) {
                if (dataSrc.source) {
                    for (let j = 0; j < dataSrc.data.length; j++) {
                            ...
                            if (matches) {
                                const mTags = {
                                    metric: !mConfig.expression
                                        ? dataSrc.data[j].metric
                                        : mLabel,
                                    ...dataSrc.data[j].tags,
                                };
                                for (
                                    let ii = 0, len = matches.length;
                                    ii < len;
                                    ii++
                                ) {
                                    const key = matches[ii].replace(
                                        /\{|\}/g,
                                        '',
                                    );
                                    if (mTags[key]) {
                                        ...
                                        if (
                                            !aliasMap[key].includes(mTags[key])
                                        ) {
                        ...
                        const tempMetric = !mConfig.expression
                            ? dataSrc.data[j].metric
                            : hasToT
                                ? midExToT
                                : mid;
                        ...
                        if (isMultiByQuery) {
                            tags = {
                                query_group: qid + '~' + gLabel,
                                ...dataSrc.data[j].tags,
                            };
                        } else {
                            tags = {
                                metric_group:
                                    mConfig.id + '~' + tempMetric + alias,
                                ...dataSrc.data[j].tags,
                            };
                        }
                        let x = xTemp;
                        ...
                        for (let k = 0; k < tagKeys.length; k++) {
                            const key = tagKeys[k];
                            const tagValue =
                                key === 'metric_group' || key === 'query_group'
                                    ? tags[key]
                            ...
                            if (multiConf.x && x.indexOf(key) !== -1) {
                                ...
                                if (
                                    multiConf.x[key] &&
                                    !multiConf.x[key].values.includes(tagValue)
                                ) {
                            ...
                            if (multiConf.y && y.indexOf(key) !== -1) {
                                ...
                                if (
                                    multiConf.y[key] &&
                                    !multiConf.y[key].values.includes(tagValue)
                                ) {
                        ...
                        let srcIndex = lookupData[y][x].results.findIndex(
                            (d) => d.source === dataSrc.source,
                        );
                        if (srcIndex === -1) {
                        ...
                        lookupData[y][x].results[srcIndex].data.push(
                            dataSrc.data[j],
                        );
                    }
                }
        ...
        if (Object.keys(aliasMap).length > 0) {
            for (const key in aliasMap) {
                ...
                aliasObj[key] =
                    aliasObj[key].length > 20
                        ? aliasObj[key].slice(0, 20) + '...'
                        : aliasObj[key];
            }
            let lookupStr = JSON.stringify(lookupData);
            let multiConfStr = JSON.stringify(multiConf);
            for (const key in aliasObj) {
                if (!aliasObj[key]) {
                    continue;
                }
                const _regex = new RegExp('{{' + key + '}}', 'gi');
        ...
        for (const tag in multiConf.x) {
            if (multiConf.x.hasOwnProperty(tag)) {
                multiConf.x[tag].values.sort(
                    this.utils.sortAliasforMultigraph(multiConf.x[tag].sortAs),
                );
        ...
        for (const tag in multiConf.y) {
            if (multiConf.y.hasOwnProperty(tag)) {
                multiConf.y[tag].values.sort(
                    this.utils.sortAliasforMultigraph(multiConf.y[tag].sortAs),
                );
        ...
        for (let i = 0; i < yCombine.length; i++) {
            for (let j = 0; j < xCombine.length; j++) {
                if (
                    (multiConf.layout === 'grid' || lookupData[yCombine[i]]) &&
                    !results[yCombine[i]]
                ) {
                    results[yCombine[i]] = {};
                }
                if (
                    ...
                    multiConf.layout === 'grid'
                ) {
                    results[yCombine[i]][xCombine[j]] = {};
                }
                if (
                    lookupData[yCombine[i]] &&
                    lookupData[yCombine[i]][xCombine[j]]
                ) {
                    results[yCombine[i]][xCombine[j]] =
                        lookupData[yCombine[i]][xCombine[j]];
                }
    ...
    getGroupByTags(queries: any[]): string[] {
        ...
        for (let i = 0; i < queries.length; i++) {
            ...
            if (!query.settings.visual.visible) {
                continue;
            }
            for (let j = 0; j < query.metrics.length; j++) {
                ...
                if (metric.groupByTags && metric.groupByTags.length) {
                    ret = ret.concat(
                        metric.groupByTags.filter((item) => ret.indexOf(item) < 0),
                    );
                }
            }
        }
    ...
    updateMultigraphConf(groupByTags: string[], multigraph: any) {
        ...
        if (multigraph) {
            if (groupByTags.length) {
                ...
                multigraph.chart = multigraph.chart.filter(
                    (item) =>
                        item.key === 'metric_group' ||
                        item.key === 'query_group' ||
                        groupByTags.includes(item.key),
                );
                for (let i = 0; i < groupByTags.length; i++) {
                    if (
                        multigraph.chart.findIndex(
                            (t: any) => t.key === groupByTags[i],
                        ) > -1
                    ) {
                        continue;
                    }
    ...
    removeEmptyRowsColumns(results: any) {
        ...
        for (let r = 0; r < rowKeys.length; r++) {
            ...
            for (let c = 0; c < colKeys.length; c++) {
                if (
                    results[rowKeys[r]][colKeys[c]].results &&
                    results[rowKeys[r]][colKeys[c]].results.length
                ) {
                    isEmpty = false;
                    break;
                }
        ...
        for (let c = 0; c < colKeys.length; c++) {
            ...
            let canDelete = true;
            for (let r = 0; r < rowKeys.length; r++) {
                const row = tmpResults[rowKeys[r]];
