    ...
    */
    checkLoginExpiration() {
        if (this.loginCheckInProgress) {
            ...
            return this.authService.getCookieStatus().pipe(
                tap((res) => {
    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<any> {
        return next.handle(request).pipe(
            catchError((error) => {
                ...
                const url = error.url
                    ? error.url
                    : error.error.currentTarget.__zone_symbol__xhrURL;
                const authConfig = this.appConfig.getConfig().auth;
                if (
                    url.indexOf('/config') === -1 &&
                    url.indexOf(authConfig.heartbeatURL) === -1 &&
                    [401, 403].includes(error.status)
                ) {
                    return this.checkLoginExpiration().pipe(
                        switchMap((res) => {
